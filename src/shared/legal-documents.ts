  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "メールアドレスとパスワード、またはGoogleアカウントで登録します。招待コードはありません。パスワードを忘れた場合は、登録メール宛に再設定用の案内を送れます。認証情報は第三者に貸与・共有しないでください。セッションはCookieで維持します。詳細はプライバシーポリシーを見てください。",
        },
      ],
    },
    {
  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "取得する情報は、アカウント（メール、パスワードのハッシュ、任意の表示名、Googleアカウントの識別子。プロフィール画像は保存しません）、セッション、規約への同意、年齢確認（生年月日は確認に成功したときだけ）、パスワード再設定の短命トークン、飲酒記録、マイドリンク、セラー、ノート、加工後の写真、AI利用回数、任意の位置情報（店名と座標）、設定から送るご意見・ご要望（種類、本文、任意の添付画像）、バックアップ、運用ログです。登録・ログイン時のボット対策（Cloudflare Turnstile）では、確認トークンを検証します。トークンは保存しません。端末だけの設定（外観、触感、動き、写真の既定、認識のON/OFF、現在地を記録する）はサーバーに送りません。",
        },
        {
          type: "p",
          text: "目的は、認証とパスワード再設定、設定表示、満20歳以上であることの確認、記録の保存と集計、写真の本人への配信、ラベル等の候補入力、ご意見・ご要望の受付と運営者への通知、不正防止（ボット対策と回数の上限）、障害対応、バックアップ、同意の記録に限ります。販売、広告配信、他ユーザーへの公開には使いません。上限の数値は公開しません。",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "インフラはCloudflareです（Workers、D1、R2、Workers AI、AI Gateway、Turnstile）。パスワード再設定の案内メールと、ご意見・ご要望の通知メールはResendへ委託します。Googleログインでは認証のためGoogleへリダイレクトします。データは日本国外のサーバで処理されることがあります。",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "法令に基づく場合を除き、個人情報を第三者に販売・提供しません。Cloudflare（インフラとボット対策のTurnstile）、Resend（パスワード再設定メールとご意見の通知）、Google（ログイン）、および（写真からの推定時）その先のモデル提供者は、サービス提供のための委託先です。",
        },
      ],
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "アカウントがある間、サービス提供に必要なデータを保管します。パスワード再設定トークンは約1時間で失効します。未紐付け写真は作成から24時間で削除します。アカウント削除は設定の「アカウントを削除」から、本人確認のうえ受け付けます。受け付けるとこのアカウントは使えなくなり、通常の保存データは削除します。写真の実体の削除は順次行います。設定から送ったご意見・ご要望の本文と添付画像は、送信者と結びつかない形（ユーザーIDを外した状態）で残します。通知メールの控えは運営者の受信箱に残ることがあります。データベースのバックアップは約14日保持する設計ですが、本番の適用状況やTime Travelなどにより、それより長く残ることがあります。全部が14日以内に消えるとは限りません。削除後に同じメールやGoogleアカウントで登録した場合は新しいアカウントになり、以前のデータは引き継ぎません。法令に基づく開示等の請求先は、本ポリシー掲載の連絡先（承認後）です。",
        },
      ],
    },
    {
