...
/** 端末が作って原本と一緒に送る一覧用サムネの上限（`kind=cutout` は透過 PNG なので大きめ） */
export const PHOTO_THUMB_MAX_BYTES = 512 * 1024;
...
/** 画面上の並びは棚（種類ごと）→ 管理一覧。「1 本ずつ」は管理一覧と重複するため廃止（`?view=one` は管理一覧に読み替える） */
export const CELLAR_LIST_VIEWS = ["type", "list"] as const;
...
export const DEFAULT_CELLAR_LIST_VIEW: CellarListView = "type";
...
export const SOCIAL_INVITE_CREATE_RATE_MAX = 10;
export const SOCIAL_INVITE_CREATE_RATE_WINDOW_MS = 60 * 60 * 1000;
export const SOCIAL_REQUEST_RATE_MAX = 20;
export const SOCIAL_REQUEST_RATE_WINDOW_MS = 60 * 60 * 1000;
export const SOCIAL_REACTION_RATE_MAX = 60;
export const SOCIAL_REACTION_RATE_WINDOW_MS = 10 * 60 * 1000;
export const SOCIAL_SHARE_RATE_MAX = 40;
export const SOCIAL_SHARE_RATE_WINDOW_MS = 60 * 60 * 1000;
