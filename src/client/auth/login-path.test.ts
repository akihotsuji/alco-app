...
import { hrefWithRedirect, loginPathFor } from "./login-path.ts";
...
describe("hrefWithRedirect", () => {
  it("redirect が無ければパスだけ返す", () => {
    expect(hrefWithRedirect("/login", null)).toBe("/login");
    expect(hrefWithRedirect("/signup", "")).toBe("/signup");
  });

  it("redirect があればクエリに載せる", () => {
    expect(hrefWithRedirect("/signup", "/logs/new")).toBe("/signup?redirect=%2Flogs%2Fnew");
    expect(hrefWithRedirect("/login", "/cellar?view=archive")).toBe(
      "/login?redirect=%2Fcellar%3Fview%3Darchive",
    );
  });
});
