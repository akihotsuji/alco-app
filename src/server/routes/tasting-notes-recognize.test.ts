...
import { noteRecognizeResponseSchema } from "@/shared/note-recognize.ts";
...
function postRecognize(
  ...
  return app.request("/api/tasting-notes/recognize", {
    method: "POST",
...
describe("POST /api/tasting-notes/recognize", () => {
  ...
  it("JPEG を読み取り、検証済み fields と remainingToday を返す", async () => {
    const ctx = await createTestApp({
      noteRecognizer: createStubLabelRecognizer(async () => ({
    ...
    const body = noteRecognizeResponseSchema.parse(await res.json());
    expect(body.provider).toBe("workers-ai");
    expect(body.remainingToday).toBe(AI_RECOGNIZE_DAILY_LIMIT - 1);
    expect(body.fields.drinkName?.value).toBe("サンプル赤");
    expect(body.fields.drinkType?.value).toBe("wine");
  ...
  it("モデル出力が壊れていても 200 で空 fields。回数は加算する", async () => {
    const ctx = await createTestApp({
      noteRecognizer: createStubLabelRecognizer(async () => "definitely not json"),
    });
    const a = await session(ctx.app, "a@example.com");
    const res = await postRecognize(ctx.app, a.cookie, makeJpeg(200, 250));
    expect(res.status).toBe(200);
    expect(noteRecognizeResponseSchema.parse(await res.json()).fields).toEqual({});
    expect(await usageCount(ctx, a.userId)).toBe(1);
  });
  ...
  it("範囲外の欄は省く", async () => {
    const ctx = await createTestApp({
      noteRecognizer: createStubLabelRecognizer(async () => ({
        drinkName: { value: "ok", confidence: 0.8 },
    ...
    const res = await postRecognize(ctx.app, a.cookie, makeJpeg(200, 250));
    const body = noteRecognizeResponseSchema.parse(await res.json());
    expect(body.fields).toEqual({ drinkName: { value: "ok", confidence: 0.8 } });
  });
  ...
  it("20ms でタイムアウトしたら 502。加算しない", async () => {
    const ctx = await createTestApp({
      ...
      noteRecognizer: createStubLabelRecognizer(() => new Promise(() => {})),
    });
    const a = await session(ctx.app, "a@example.com");
    const res = await postRecognize(ctx.app, a.cookie, makeJpeg(200, 250));
    expect(res.status).toBe(502);
