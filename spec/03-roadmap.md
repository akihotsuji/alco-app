...
- [ ] マイドリンク（プリセット）の登録・1タップ記録（**ホームのみ**。チップの水位線 M-07 / M-08、数字カウント M-09、週マス M-17、休肝ピル M-18）。同じタスクで **ホームの導線と中央タブ**を (c) に切り替える: 中央タブとホームのカメラ円ボタンは `photo-edit` を開き「使う」で写真付き `log-new`（キャンセルは元の画面、着地・現在地ハイライトなし）、今日カード → 今日の `/logs`、カード内「今週 ›」→ 週サマリー（[screen-designs/00-common.md](screen-designs/00-common.md) 1.2、[02-home.md](screen-designs/02-home.md)）
...
- [ ] 設定「操作」節: 触感フィードバック（既定 OFF）・動きを減らす（[screen-designs/06-settings.md](screen-designs/06-settings.md) S8 / S9。3-07）
...
- [ ] APIテスト・コンポーネントテスト（consume / restore / recognize の認可・上限含む）
