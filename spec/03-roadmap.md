...
- [x] **開栓・貯蔵庫・復元**: 「開栓する」（記録は作らない → 棚へ戻りトースト undo。到着時に本数カウントダウン + 抜けた段の棚板ハイライト M-10）、`/cellar/archive`（月見出し・原色のまま）、「開栓の記録を取り消す」→ 4-03
...
- [ ] **5.5-02 セラー写真処理の安定化**: 起票済み [Issue #48](https://github.com/akihotsuji/alco-app/issues/48) を追跡Issueとして、現行コードで成立する仮説を再確認し、P0/P1を1関心事の子Issueへ分割して順に解決する。WebGPU 経路は実装済みだが本番既定は WASM（実機比較未了）。WASMマルチスレッド・モデル交換は実測後に別Issue（2026-09-07: P0/P1 実装済み。2026-09-14: 推論入力と角度補正を追加）
...
- [x] vite-plugin-pwa導入: manifest、アイコン一式（キャラクター `default` をライト地に載せてビルド時生成）、スタンドアロン表示、テーマカラー → [features/pwa.md](features/pwa.md)（6-01。2026-09-08。表示名とアイコン地は後続で改訂）
...
- [x] 本番用リソース作成: Workers環境分離（wranglerのenv機能）、本番D1・本番R2 → [features/production-env.md](features/production-env.md)（7-01。2026-09-09）
- [x] GitHub Actions: mainマージ→dev自動デプロイ（開発 Phase で 7-02 の dev 側を先行。正本は [features/dev-deploy-ci.md](features/dev-deploy-ci.md)）
- [x] GitHub Actions デプロイパイプライン: タグ/手動承認→本番デプロイ → [features/deploy-prod.md](features/deploy-prod.md)（7-02。2026-09-09。Environment 必須レビューアと初回デプロイはオーナー）
...
- skill: `release` — リリース手順（チェックリスト実行→本番デプロイ→動作確認→ロールバック手順）（7-08。2026-09-09）
...
- [x] 利用規約・プライバシーポリシー作成 → [legal.md](legal.md) / [features/legal.md](features/legal.md)（8-01。2026-09-21 に友達・近況を文面へ反映。草案。オーナー承認待ち）
...
- [x] 友達・近況共有（招待・相互承認・フィード・リアクション・アプリ内通知）→ [features/friends-social.md](features/friends-social.md) / [screen-designs/13-friends.md](screen-designs/13-friends.md)
