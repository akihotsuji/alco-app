...
- [ ] 週/月サマリー: 推移グラフ（軽量なチャートライブラリ。初回 1 回だけ伸びる M-19）、休肝日表示
...
- [ ] **棚（陳列）**: 地色の上に **ガラス風の棚板**、**種類ごと / 1 本ずつ**の表示切替、`kind`（切り抜き / 長方形）の描き分け、写真なしは種類別シルエット、種類・状態・検索フィルタ、空状態（棚板ハイライト M-28、直前に操作した本の M-32）
- [ ] **消費・貯蔵庫・開栓・復元**: 消費ダイアログ（記録 1 杯を同時作成 → その日の日別へ、トースト undo）、`/cellar/archive`（月見出し・減彩）、「開栓する」（ピルの切替 M-10・ボタン退場 M-11・棚板ハイライト M-12）、「セラーに戻す」
