...
| `--header-h` | 56px | ヘッダー |
| `--tab-h` | 76px | 下部タブ＋余白（中央円の浮きとラベル「飲酒を記録」。ラベルは 1 行） |
| `--tab-center-size` | 52px | 中央「飲酒を記録」タブの円 |
| `--tab-center-lift` | 8px | 中央タブの円がタブバー上端から浮く量。下ラベル「飲酒を記録」は 1 行 |
...
| 貯蔵庫（アーカイブ）のボトル | 同じ棚。写真は減彩しない（原色のまま。`filter: none`）。右上に消費日ピル |
| AI 読み取りの印 | フィールド右端に `pill.ai`（20px、`sparkles` アイコン + 「AI」、primary 文字）。ユーザーが編集したら消す |
| AI 読み取り中の印 | 読み取り中、AI が入れる可能性のある欄（空欄・直前の AI 印の欄。触った欄は除く）の右端に `pill.ai.ai-pending`（同じ 20px。小さなスピナー + 「読み取り中」、muted 文字）。結果が来たら消え、入った欄だけ `pill.ai` に置き換わる（2026-09-10。酒記録の `log-new` / `log-edit`） |
...
| `--tab-h` | **76px**（72 → 76 に改訂） | 同じ | 中央円の浮きとラベル分（ラベルは 1 行） |
...
- [x] 1-07 / 1-08 追補（キャラクター・陳列・写真・中央タブのトークン）のオーナー承認（2026-09-06）
