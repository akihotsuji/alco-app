...
### 4.4 種類グリッドの並べ替え（`bottle-type-grid`）

エージェントは実機 PWA を持たない。以下はオーナー実機の再現手順。推測で合格にしない。

必須動画: 長押し → 同一マス内で追従 → 横移動 → 段またぎ → 端で指を止める → ドロップ。

| 環境 | 確認 |
|---|---|
| iPhone・ホーム画面追加 PWA | 長押し、指追従、段またぎ、端停止スクロール、着地、OS メニューが出ない、復帰後のスクロール |
| iPhone・Safari 通常表示 | 同じ操作、viewport / safe area 差 |
| Android・インストール PWA | 同じ操作、端末戻る、対応端末での触感設定 |
| Android・Chrome 通常表示 | 同じ操作、通常スクロール |
| PC ブラウザ | マウス、ホイール、キーボード、Escape、補助モード |

記録する項目: 機種・OS・ブラウザ・standalone・画面幅・アプリのビルド ID。データは 0/1/4/5/12 本、縦に長い 40 本以上、ページングを含む 100 件超。切り抜き PNG / 通常写真 / 写真なし / 長い名前を混在。個人・共有、検索あり/なし、ライト/ダーク、縦/横、OS/アプリの動き軽減。
...
契約は [features/pwa.md](features/pwa.md) 6.2。友達 2 アカウント（テスト用）で、B から A へ申請または反応を送り、A の未読を作る。エージェントは実機 PWA を持たない。推測で合格にしない。

| 環境 | 手順 | 期待 |
|---|---|---|
| iPhone・ホーム画面追加 PWA（通知未許可のまま） | A で起動 → 前面のまま B から反応 → A をいったん背面にして戻す | アイコンに数字は出ない（WebKit は通知許可が無いと出さない）。アプリが落ちない・固まらない。F2 のバッジは出る |
| Android・インストール PWA | 同じ手順 | 数字・ドットは出ない想定（Chrome Android は Badging API を表示しない）。落ちない・固まらない |
| 共通 | A で未読がある状態からログアウト → B でログイン | A の件数が残らない（出ていたなら消える） |
