...
契約は [features/web-push.md](features/web-push.md)。本番（または dev）に VAPID 鍵を入れてから行う（[secrets.md](secrets.md)）。友達 2 アカウントで、B から A へ申請・承認・反応を送る。エージェントは実機 PWA を持たない。推測で合格にしない。

| 環境 | 手順 | 期待 |
|---|---|---|
| iPhone・ホーム画面追加 PWA（iOS 16.4+） | A で設定 → 「友達のお知らせを通知する」を押す → 許可 → アプリを完全に閉じる → B から反応 | ロック画面に「酒のしおり／あなたの共有にリアクションがありました」。名前・お酒・リアクションの種類・写真は出ない。アイコンに未読数。タップで通知一覧が開く |
| iPhone・Safari タブ | 設定を開く | スイッチ無効。「ホーム画面に追加した酒のしおりで使えます」。許可ダイアログは出ない |
| Android・インストール PWA | 同じ手順（申請・承認も） | 通知が届き、アイコンにドット。タップで通知一覧 |
| 共通・拒否 | 許可ダイアログで「許可しない」 | スイッチ無効 + 端末設定の案内。以後、起動しても許可を求めない |
| 共通・オフ／ログアウト | スイッチをオフ、またはログアウト → B から反応 | A の端末に届かない |
| 共通・起動 | 未許可の端末でアプリを開く・友達タブへ移る | 許可ダイアログが出ない（通知画面の N1 カードは出てよい） |

### 4.8 アイコン長押しのショートカット

契約は [features/pwa.md](features/pwa.md) 3.1。反映前にインストールした端末は、いったんホームから削除して追加し直してから行う。エージェントは実機 PWA を持たない。推測で合格にしない。

| 環境 | 手順 | 期待 |
|---|---|---|
| Android・インストール PWA | アイコンを長押し →「飲酒を記録」 | 記録入力（見出し「お酒を記録」）が写真なしで開く。カメラは起動しない。「←」で今日の日別記録 |
| Android・未ログイン | ログアウト → 長押し →「飲酒を記録」→ ログイン | ログイン後に記録入力が開く |
| iPhone・ホーム画面追加 PWA | アイコンを長押し | 「飲酒を記録」は出ない（Safari 非対応）。通常のタップ起動は今までどおり |
