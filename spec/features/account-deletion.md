...
1. 削除要求 ID を作る
2. 当人の `photos.r2_key` と未コミット予約キーを写真削除タスクへ `INSERT SELECT` する（JS へ全件を読み出さない）
...
受付成功後は Query をキャンセルしてキャッシュを捨て、複数タブへ通知する。アカウントに紐づく下書き・ガイド進捗・切り抜き診断は捨てる。テーマ・触感・動き・写真既定など個人データを含まない端末設定は残す。`localStorage.clear()` はしない。
