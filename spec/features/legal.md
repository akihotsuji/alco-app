...
- データマップと草案（酒類・20 歳、免責、Cloudflare への国外移転、写真の権利、飲酒記録の扱い、共有セラー、友達・近況、退会、ご意見、AI 照合、PWA）
...
- 更新時の再同意ゲート（作らない。2026-09-22 決定。公開前で利用者はオーナーのみ）
- 権利侵害の報告ボタンと公開メール（作らない。申出は `/settings/feedback` の種類「その他」。ログインかつ年齢確認済み。アカウントの無い人は使えない。公開前・友達限定では許容。[legal.md](../legal.md) 1.1）
- 英語版
...
|---|---|---|
| 言語 | 日本語のみ | 公開対象はまず日本 |
| 事業者 | 公開ページは「本サービスの運営者（個人）」。氏名・住所は掲載しない。住所は開示請求があれば遅滞なく開示。公開メールは置かない | 2026-09-22 決定。個人名・住所・メールは作らない |
...
| 再同意 | 求めない。再同意ゲートは作らない。版と施行日を公開ページに出す。公開前で利用者はオーナーのみ。既存ユーザーはロックしない | 2026-09-22 決定 |
| 既存ユーザー | 同意行が無くてもアプリは使える（個人利用で既に登録済みの移行） | 既存アカウントをロックしない |
| Cookie バナー | 置かない | セッション Cookie のみ。日本向け第一歩 |
| 電気通信事業法 | この段階では届出しない。DM・コメント・返信は無い。DM または自由記述のメッセージングを足すときだけ見直す。法律意見ではない | 2026-09-22 決定 |
| 権利侵害の申出 | 設定「ご意見・ご要望」種類「その他」（`/settings/feedback`。ログインかつ年齢確認済み）。報告ボタンも公開メールも新設しない | 2026-09-22 決定。アカウントの無い人は使えない。公開前・友達限定では許容 |
| 飲酒記録 | 要配慮個人情報としては扱わない。利用者が任意に入れる生活記録。診断・治療・保険査定には使わない | 2026-09-22 決定。医療目的には用いない |
