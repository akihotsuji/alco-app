...
オーナー判断待ちの選択肢。採用したものだけ上の節に移す。数値は 2026-09 時点の公開資料。
...
### 15.3 案 B: 抽出モデルを Gemini 3.5 Flash-Lite にする
...
1. 検証済み対応表の拡充（**本 PR で実施**。San Fereolo のような `Dogliani` / `Langhe` は表で イタリア に決まる）
2. `unverified_guess` の国を捨てずに「候補: イタリア」チップとして欄の下に出し、タップで採用する（自動入力はしない。§6 の方針は維持）。読み取ったのに空欄に見える問題を解消する
...
### 15.7 案 F: 小さな調整（すぐできる）

- `maxOutputTokens` 4096 → 1024（抽出 JSON は数百トークン）
- 認識用 JPEG の長辺を表示用と別に 1024px 程度へ落とす（入力トークンと転送量が減る）
- 検索予算 4 秒 → 3 秒、または案 A と組み合わせて予算超過時も抽出結果だけ即返す

### 15.8 推奨順

A（二段階）→ F（小調整）→ E-2（候補チップ）→ B（Flash-Lite 比較）→ 必要なら C。D は予備。
