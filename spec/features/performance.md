...
- 先読み GET は 10 秒で打ち切る。失敗した Promise を本バンドルが待たず、通常の `fetch` にフォールバックする。**先読みの応答が 401 のときも使わず通常の `fetch` で取り直す**（未ログインで `/` や `/logs/new` を開くと、使われない 401 が残る。ログイン後の最初の query がそれを受け取るとセッション切れと判定され、サインアウトされてしまう）。画面 `import()` の失敗は握りつぶして未処理拒否にしない（本体の Error Boundary / `vite:preloadError` が復旧する）
...
- ボトル一覧は個人セラーを作らず `user_cellar_slots` / 所属のサブクエリで絞る。写真メタと表示名は並列
- ノート／記録一覧はページとサムネ用 photos を同じ壁時計で取る（IN サブクエリ）
- 記録サマリーは `GROUP BY drunk_on`
- ボトル作成の二重メンバー確認と表示名の二重取得を削除
- ボトル／記録／ノートの更新・開栓・戻しは write 後の再 SELECT / `getOwn*` をやめ、RETURNING と既知の行で返す
- 写真 POST は日次上限・所有・容量を並列。ボトルは `requireAccessibleBottle` を使い回す
