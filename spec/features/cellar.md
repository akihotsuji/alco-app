...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| B1 | 写真（表裏） | 横並びサムネ + 操作パネル | なし | 保存先の次。ありは「写真を編集」。なしは「写真を撮る / 選ぶ」。裏面なしは「＋ 裏ラベル」。パネル: 切り抜きを調整 / 撮り直す / 写真を選び直す / 写真を削除。閉じたらフォーカス復帰。明示タップ以外では起動しない。アプリ内導線は `?camera=1` を付けない。棚・一覧・サムネに使うのは**表面だけ** |
...
| B4 | 種類 | 選択済みボタン + パネル | **赤ワイン** | 必須。全 12 種。新規に未選択時の確定値を足さない。AI は色・泡・箔から具体種を優先。ユーザーが先に触っていなければ選択を変える |
| B5 | 本数 | − / 数値 / ＋ | 1 | 1〜12。数値直接入力可。同じ写真（表 + 裏）・属性で N 行 |
