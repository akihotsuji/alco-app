...
| B2 | ラベル読み取り帯 | 自動 + 手動 | — | 表面の「使う」で切り抜く前の 2:3 JPEG ができた時点で `POST /api/bottles/recognize`（**表面だけ**。背景除去・アップロードを待たない。同じ JPEG への要求は 1 リクエストにまとめる）。裏面を追加 / 差し替え / 削除しても **自動では再リクエストしない**。成功帯かつ裏面 JPEG があるとき右端に「裏面も含めて読み取る」。失敗帯には「再読み取り」。どちらもタップで表 + 裏（裏が無ければ表だけ）を再リクエストする（回数は加算される。結果は空欄と AI 印の欄だけ更新。触った欄は守る）。設定 `cellar.recognize` が OFF なら帯もリクエストも出さない |
...
6. 裏面を追加 / 差し替え / 削除しても自動では再リクエストしない。成功帯の「裏面も含めて読み取る」または失敗帯の「再読み取り」で、表面の JPEG + 裏面の JPEG（あれば）を送る。古い要求の結果が後から返っても無視する（最新の要求だけ反映）
7. 「再読み取り」「裏面も含めて読み取る」は同じ画像で再リクエストする。クライアントの同一 JPEG まとめ（B2）はこれらの手動操作のときだけ解除する

編集画面（3.5）を開いただけでは読み取らない。その編集セッションで新規写真を追加 / 差し替えたときだけ、追加時と同じ補完と上書き保護を適用する（表面を差し替えずに裏面だけ追加した場合は読み取らない。保存済みの表面 JPEG が端末に無いため）。設定 OFF なら開始しない。
...
| E42 | 裏面を追加した | 自動では再読み取りしない。成功帯に「裏面も含めて読み取る」が出る。押したあと AI 印の欄だけ更新。ユーザーが触った欄は守る |
...
| 裏面 | 読み取りの開始 | 表面の「使う」だけ自動。裏面追加後は成功帯の「裏面も含めて読み取る」（失敗帯は「再読み取り」） | 先に表面を撮る導線。裏面を付けるたびに回数を消費しない |
