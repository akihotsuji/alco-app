...
| 項目 | 規則 |
|---|---|
| 行 | 1 枚の写真 = 1 行 = 1 銘柄。本数 N 可（1〜12）。上限 **20 行**。行の項目は品名 / 種類 / 本数 / 生産者 / 生産国 / ヴィンテージ / 品種（購入日・価格・場所・メモは持たない。あとで `bottle-edit`） |
| 写真 | `bottle-new` と同じパイプライン（2:3、`cellar`、切り抜き既定 ON、キャラ合成なし）。「使う」直後に未紐付け `POST /api/photos`。行を外す × と破棄で未紐付けの写真を `DELETE /api/photos/:id` |
| ラベル読み取り | 行ごとに `POST /api/bottles/recognize`（切り抜く前の 2:3 JPEG）。結果はその行の空欄、または直前の AI 印の欄に入れて AI 印。設定 `cellar.recognize` OFF なら呼ばない。1 日 10000 回（許容上限 MAX）の上限はそのまま（超えた行は失敗帯、保存は止めない） |
...
| 成功（`fields` が 1 つ以上） | 「ラベルから読み取りました。内容を確認して保存してください」。空欄と直前の AI 印の欄に候補。入った欄に AI 印 |
...
編集画面（3.5）を開いただけでは読み取らない。その編集セッションで新規写真を追加 / 差し替えたときだけ、追加時と同じ補完と上書き保護を適用する。設定 OFF なら開始しない。
...
| T5 | プロパティ | 順: 品名 / 種類 / ヴィンテージ / 品種 / 生産国 / 生産者 / 購入日 / 価格 / 購入場所 / 保管日 / 保管場所 / メモ。値が空の行は出さない（未入力年を「NV」と書かない）。日付は「2026年9月5日」。価格は `¥3,800`（整数・桁区切り）。長い値は項目名の下に左寄せ | data-model 6.3 |
...
| ラベル AI | 認証必須。プロンプトにユーザー文を入れない。出力は Zod。画像・結果を保存・ログしない。Cloudflare 外に送らない。日次 10000 回（許容上限 MAX）。設定 OFF はクライアント抑制のみ（サーバー上限は残す） |
