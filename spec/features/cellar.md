...
| B6 | 詳細 | 折りたたみ | 閉じる | セクション順: **ボトル情報**（生産者・産地・ビンテージ）→ **保管情報**（保管日・保管場所）→ **購入情報**（購入日・購入価格・購入場所）→ **メモ**。ラベルはすべて入力の上。狭い画面は 1 列（産地/ビンテージ、購入日/価格を無理に 2 列にしない）。保管日の初期値は登録日（JST、詳細を閉じたままでも保存。未変更なら保存日の当日）。保管場所の初期値は「自宅セラー」（プレースホルダではなく実値）。購入日は自動設定しない。編集では保存済みの保管日・保管場所を出し、空欄を補完しない。AI が **生産者 / 産地 / ビンテージ**のいずれかを入れたら自動で開く |
...
| 項目 | 規則 |
|---|---|
| 行 | 1 枚の写真 = 1 行 = 1 銘柄。本数 N 可（1〜12）。上限 **20 行**。行の項目は銘柄名 / 種類 / 本数 / 生産者 / 産地 / ビンテージ（購入日・価格・場所・メモは持たない。あとで `bottle-edit`） |
...
| T5 | プロパティ | 順: 銘柄名 / 種類 / ビンテージ（無ければ「NV」）/ 産地 / 生産者 / 購入日 / 価格 / 購入場所 / 保管日 / 保管場所 / メモ。銘柄名・種類・ビンテージ以外は空行を出さない。**品種列は持たない**。価格は `¥3,800`（整数・桁区切り） | data-model 6.3 |
...
| `producer` / `origin` / `shop` / `storage` | 0〜100。trim 後に空なら `null`。作成時 `storage` 省略は「自宅セラー」。`null` 明示は空のまま | 「100文字以内で入力してください」 |
...
| 本仕様 | recognize の `abvPercent` | API は返す。**フォームに入れない**。詳細の自動オープンは生産者 / 産地 / ビンテージのみ | ボトルに度数列が無い（要件 1.3 / data-model 6.3）。画面設計 B6 を同じ PR で修正 |
