...
手持ちのボトルを **撮って棚に並べ、開栓で味わい中へ、飲み切ったら貯蔵庫へ移す**。操作は **追加**・**開栓**・**飲み切り**。1 杯の記録は作らない（2026-09-24 に味わい中を追加。正本は [bottle-tasting.md](bottle-tasting.md)）。

| ゴール | 内容 |
|---|---|
| 追加 | 「+」→ 撮影（2:3 / `cellar` / 切り抜き既定 ON / キャラなし）→ 確認して「棚に並べる」。本数 N で N 行 |
| 棚 | 地色の上にガラス棚板。切り抜き（または長方形）が立つ。種類ごと / 1 本ずつ |
| 開栓 | 確認なしで味わい中へ。記録は増えない。トーストの undo = 開栓を取り消す。飲み切ったら「飲み切った」で貯蔵庫へ |
...
| 記録・ノート写真の AI 推定 | 記録は実装済み。ノート識別は記録側 |
| 棚に残す「開栓済み」（`opened`） | 棚の陳列には混ぜない。2026-09-24 に「味わい中」としてセラー上部の補助表示に復活（[bottle-tasting.md](bottle-tasting.md)） |
...
| undo | 開栓後トーストの「取り消す」（6 秒。`restore`）と、飲み切りトーストの「取り消す」（`reopen`） |
| 表示切替 | クライアントの棚レイアウト。`?view=type\|list`（`?view=one` は `list` に読み替え。API の `view=cellar\|archive\|all` とは別） |
...
### 3.1 `bottle-list` セラー（`/cellar?view=type|list&q=&drinkType=`）

親タブはセラー。入口は下部タブ「セラー」。在庫（`sealed`）だけを出す。

| # | 項目 | 内容 | データ |
|---|---|---|---|
| C1 | 貯蔵庫 | ヘッダー左。短い名称「貯蔵庫」→ `/cellar/archive`。バッジなし | — |
| C2 | タイトル | 「セラー」+ muted「N 本」。**N はフィルタ前**の棚在庫数 | `GET /api/bottles?view=cellar` の `totalCount` |
| C3 | 追加 | 右下 FAB ピル「＋ ボトル追加」→ `/cellar/new`（撮影は自動起動しない） | — |
| C3b | まとめて追加 | ヘッダー右の短い名称「まとめて追加」→ `/cellar/batch` | — |
| C4 | 表示切替 | 「棚」「管理一覧」（「1 本ずつ」は廃止。`?view=one` は管理一覧に読み替え）。URL `?view=` と `localStorage` `cellar.listView`（既定 `type`） | クライアント |
| C4b | 管理一覧 | 1 行 = 1 本。同名統合なし。写真・銘柄・年号・国・本数 | `items[]` |
| C5 | 検索 | Input（「表示・絞り込み」の開閉の中）。品名・生産者・品種の部分一致。300ms デバウンス。最大 100 文字 | `q` |
| C6 | 種類フィルタ | Chip「種類 ▼」→ 12 種ダイアログ。単一選択。選択中は「赤ワイン ×」。**種類ごと表示では非表示** | `drinkType` |
| C8 | 棚（1 本ずつ） | 廃止（管理一覧と重複。`?view=one` は管理一覧に読み替え） | — |
| C9 | 棚（種類ごと） | 種類は 12 種の定義順。在庫 0 の種類は出さない。段は横スクロール（`scroll-snap`）。見出し「種類名 N本」+ 右「すべて見る ›」。タップで `bottle-type-grid`。棚板は本数分の幅。段の並びは `sort_order` | 初回は `GET /api/bottles?view=cellar&group=type&limit=12` の `typeShelves`。段の追加取得は種類ごとに `GET /api/bottles?view=cellar&drinkType=&limit=12&cursor=` |
| C10 | ボトル | 切り抜き 100×150（種類ごとは 72×120）+ 名前 13px 最大 2 行。`cutout` / `photo` とも縦横比を保つ。無ければ種類別シルエット | `GET /api/photos/:id/content?variant=thumb`、`thumbPhotoKind` |
| C11 | サブ行 | 1 本ずつと管理一覧は年を補助行に出す。無ければ空行（「NV」と書かない）。種類ごとでは出さない | `vintage` |
...
- `?view=` が `one` / `type` / `list` 以外、または無し → `localStorage`、それも無ければ `one`。404 にしない
...
| # | 項目 | 内容 | データ |
|---|---|---|---|
| R1 | 完了 | ヘッダー左。閉じる＝保存。未 drop の操作は中断してから、確定済みの dirty だけ送る。未変更なら通信しない。着地アニメは待たない | `PUT /api/bottles/order` |
| R2 | タイトル | 種類名 + muted「N 本」（その種類の在庫。検索中はヒット数） | `countsByType` / 取得件数 |
| R3 | 補足 | 検索なしのときだけ「長押しして並べ替え」。横に小さな「並べ替え」（補助モード開始） | — |
| R4 | 棚 | 常に 4 列 × n 段。ボトルは 1 つの CSS Grid の直接の子（`item.id` key）。棚板は装飾（`board-段番号`、`aria-hidden`、`pointer-events: none`）。最後の段が 1〜3 本でも棚板は横一杯 | `GET /api/bottles?view=cellar&drinkType=&limit=100` を cursor 完走 |
| R5 | ボトル | タイルはボタン。短いタップ → 詳細。長押し（400ms、移動 10px 未満）でその場に持ち上げ、指に追従する独立レイヤー（M-33）。挿入先が変わると周囲が FLIP で空ける（M-34）。離すと着地してローカル順を確定。検索中は並べ替え不可 | ローカル順 |
| R6 | 保存 | 完了 / 端末戻る / シートを閉じる / `pagehide`・`visibilitychange=hidden` で dirty なら 1 回送る。成功トーストなし。失敗は閉じず、操作付近に再試行を残す | `bottleIds` がその種類の sealed 全件 |
| R7 | 補助モード | ヘッダー下の「並べ替え」→ 選択 →「前へ」「次へ」→「決定」。シートの「完了」とは別。棚を覆わない。検索中は無効 | ローカル順 |

並べ替えの範囲は同じセラー・同じ酒類・在庫 `sealed` だけ。種類セクション自体や横スクロール棚の DnD は無い。
...
- 正常 drop は最終座標を同期確定 → 約 180ms で着地（`--dur-type-grid-settle`）。`pointercancel` / 予期しない capture 消失 / drag 中の Escape / 回転・viewport 変更 / 非表示は、当該ドラッグ開始直前の順へ戻す（以前の drop は残す）
- ドラッグ中はサーバーへ送らない。「キャンセルで破棄」ボタンは置かない
- 読み込み完了前・検索中・保存中・0〜1 本では開始しない
- 新規・復元・種類変更で棚に戻る本は、その種類の先頭（左上）
- iOS 風の揺れ（jiggle）は置かない（無限アニメ禁止）

保存の調停:

- `baseline` はサーバー確認済み、`items` は drop 済みローカル順、ドラッグ中プレビューは未確定
- idle かつ未変更のときだけサーバー順を取り込む。dirty 中の ID 集合変化は消えた本を残さず、順序だけの外部変更では引き戻さない
- 同一 ID 順の再試行は同じ `operationKey`。別順は新しい key。送信中の二重 close / visibility は再送しない
- 失敗は閉じずローカル順を保持。409 `reason: "set"` と対象外 ID の 404 は競合復旧（再取得成功まで操作停止。再取得失敗中はローカル順を捨てない）
- 順序競合用の revision はこの UI では新設しない。`pagehide` の非同期保存は成功を保証しない
- dirty / 操作中 / 保存中は既存の離脱保護で PWA 自動更新を止める
...
| A3 | ボトル | 棚と同じ切り抜き。**減彩しない**（原色のまま。`filter: none`）銘柄最大 2 行 + 年号。右上に「飲み切り 9/24」ピル（`finishedOn`。開栓日と同じ日は「開栓・飲み切り 9/5」） | `items[]` |
...
- 説明: 「飲み切ったボトルが日付順に並びます。開栓したボトルはセラーの上の「味わい中」にあります。」
- 並びは `finishedAt`（飲み切り日時）降順
...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| B1 | 写真（主） | 空枠 1 つ + 撮影／ライブラリ | なし | 保存先の次。空は「ボトルの写真を追加」+「撮影」「ライブラリ」。表裏の同じ空枠は並べない。空枠タップは選択パネル（カメラ即起動しない）。ありは「写真を編集」。裏面なしは「＋ 裏ラベルを追加（任意）」。パネル: 切り抜きを調整 / 撮り直す / 写真を選び直す / 写真を削除。閉じたらフォーカス復帰。明示タップ以外では起動しない。アプリ内導線は `?camera=1` を付けない。棚・一覧・サムネに使うのは**表面だけ** |
| B1b | 裏ラベル（任意） | 「＋ 裏ラベルを追加（任意）」（表面があるときだけ出す） | なし | 最大 1 枚。**編集画面（切り抜き・トリミング）は開かず**、ラベル部分だけを自動で切り出した JPEG（`kind = photo`。切り出せなければ瓶の外接矩形、瓶も見つからなければ 2:3 中央トリミング）にして即 未紐付け `POST /api/photos`。裏面だけの登録はできない（表面を削除したら裏面も外す）。サムネ + 「裏ラベル」+ 変更／削除。読み取りの材料（手動の再判定で使う）。追加だけでは B2 を自動再実行しない。棚・一覧には出さない |
| B1c | 裏ラベルの確認 | シート（新規のみ） | — | 表面を付けた直後、裏ラベルがまだ無ければ「裏ラベルも登録しますか？」。撮る／選ぶ → 裏ラベルの変換後に表 + 裏で B2 を 1 回。いいえ・閉じる・OS キャンセル → 表面だけで B2。答えが出るまで B2 を始めない（表面だけの先行リクエストもしない）。画面は [04-cellar.md](../screen-designs/04-cellar.md) B1c |
| B2 | ラベル読み取り帯 | 自動 + 手動 | — | 新規は B1c の答えのあと、編集で表面を差し替えたときは表面の「使う」で切り抜く前の 2:3 JPEG ができた時点で `POST /api/bottles/recognize`（編集は **表面だけ**。背景除去・アップロードを待たない。同じ JPEG への要求は 1 リクエストにまとめる）。裏面を追加 / 差し替え / 削除しても **自動では再リクエストしない**。裏面 JPEG があるときは「裏面も含めて読み取る」（成功帯の右端、または編集で保存済み表面だけのときは案内帯）。失敗帯には「再読み取り」。どちらもタップで表 + 裏（裏が無ければ表だけ）を再リクエストする（回数は加算される。結果は空欄と AI 印の欄だけ更新。触った欄は守る）。編集で保存済み表面しか無いときは配信画像を端末で認識用 JPEG にして `file` に載せる。設定 `cellar.recognize` が OFF なら帯もリクエストも出さない |
| B3 | 品名 | Input | 空 | 必須 1〜100。ラベルに「必須」。プレースホルダ「例：常山 わせ」。AI は **空のときだけ**入れ、右端に `pill.ai`。ユーザー編集で印が消える |
| B3s | Googleで調べる | テキストリンク | — | 品名直下。有効な品名があるときだけ。保存可否と独立。[drink-search.md](drink-search.md) |
| B4 | 種類 | 選択済みボタン + パネル | **赤ワイン** | 必須。全 12 種。新規に未選択時の確定値を足さない（入口カテゴリの根拠ある既定）。AI は色・泡・箔から具体種を優先。ユーザーが先に触っていなければ選択を変える |
| B5 | 本数 | 「本数」と − / 数値 本 / ＋ を 1 行 | 1 | 1〜12。数値直接入力可。最小では減らすを無効。同じ写真（表 + 裏）・属性で N 行 |
| B6 | ヴィンテージ | 常時 | 品名・種類・本数の次 | 未入力 / NV の既存区別を維持。推測しない |
| B6b | お酒の詳細（任意） | 折りたたみ | 閉じる | 品種・生産者・生産国・保管・購入・メモ。検証エラー時は開く。保管日の初期値は撮影日（無ければ登録日 JST。詳細を閉じたままでも保存。未変更なら撮影日または保存日の当日）。保管場所の初期値は「自宅セラー」。購入日は自動設定しない。編集では保存済みの保管日・保管場所を出し、空欄を補完しない。手入力を AI で上書きしない |
| B7 | 棚に並べる | Button 主 | 有効 | ラベル「棚に並べる（N 本）」。無効: 名前が空 / 範囲外、写真アップロード中・失敗中、保存中。無効理由は付近に出す（品名不足は「品名を入力すると登録できます」。処理中は「写真を処理しています」）。**読み取り中は無効にしない** |
...
| 項目 | 規則 |
|---|---|
| 行 | 1 枚の表面写真 = 1 行 = 1 銘柄。本数 N 可（1〜12）。上限 **20 行**（処理待ち・失敗行も枠に含める）。行の項目は品名 / 種類 / 本数 / 生産者 / 生産国 / ヴィンテージ / 品種（購入日・価格・場所・メモは持たない。あとで `bottle-edit`）。選択順を維持し、処理完了順で並べ替えない |
| ライブラリ取り込み | OS の選択結果を受けた直後、受け付けた枚数ぶんを安定した行 ID で一覧に足す。変換前の行も残す。上限を超えた分は足さず、「受け付けた枚数」と「追加しなかった枚数」を画面に出す（黙って切らない）。アプリ側に 5 枚制限は無い。OS の選択画面が枚数を制限する場合はアプリの不具合と分けて扱う |
| 写真の工程 | 行ごとに `待機中 / 変換中 / アップロード中 / 完了 / 失敗` を持つ。変換は原則 1 件ずつ、アップロードは同時最大 2 件（裏面追加・手動再試行も同じ画面内で枠を共有）。残りは待機。変換と通信は上限の範囲で重ねる。全工程の巨大な直列にはしない |
| 写真 | 表面は `bottle-new` と同じパイプライン（2:3、`cellar`、切り抜き既定 ON、キャラ合成なし）。変換成功後に未紐付け `POST /api/photos`。行を外す × と破棄で未紐付けの写真を `DELETE /api/photos/:id`。保存用 JPEG / WebP / PNG は送信前に `blob.size` を検査し、1MiB に収まらなければ送らない（品質と解像度を段階的に下げ、最終結果を再検査する。切り抜きの透過は維持） |
| 裏面 | 行ごとに任意で **最大 1 枚**（G2b「裏面」。撮る／選ぶ）。編集画面は開かず、ラベル部分を自動で切り出した JPEG（`kind = photo`）でアップロード（同時 2 件枠を共有）。追加だけでは自動再読み取りしない。行を外す × と破棄で表面と一緒に `DELETE` |
| 切り抜き | 編集プレビューの推論は「最新 1 件だけ残す」。保存・バッチの別写真は FIFO で捨てない。切り抜き失敗でも通常写真（JPEG）が取れればアップロードは続ける。切り抜き失敗を写真アップロード失敗にしない |
| ラベル読み取り | 行ごとに `POST /api/bottles/recognize`（切り抜く前の 2:3 JPEG。**自動は表面だけ**）。結果はその行の空欄、または直前の AI 印の欄に入れて AI 印。ユーザーが触った欄と遅れて届いた結果で消さない。設定 `cellar.recognize` OFF なら呼ばない。1 日 10000 回（許容上限 MAX）の上限はそのまま（超えた行は失敗帯、保存は止めない）。**同時に走らせる読み取りは 2 行まで**（残りは待ち行列。中断済みの待ちは API を呼ばない） |
| 再読み取り | 行の上部の帯に出す。失敗帯（上限・502・タイムアウト・`fields` 空）に「再読み取り」リンク。成功帯かつ裏面があるときは「裏面も含めて読み取る」。タップした**その行だけ**を表 + 裏（裏が無ければ表だけ）で再リクエストする（回数は加算される）。読み取り中は無効。成功帯に汎用の「もう一度読み取る」は出さない。AI 失敗でも写真と手入力は残し、保存を止めない |
| 再試行 | 失敗した工程だけを再実行する。成功済みの変換・アップロード・AI は繰り返さない。「写真を選び直す」はその行の写真だけ差し替える。連打しても同じ行を二重処理しない |
| 連続撮影 | 「使う」の **同じタップ**で次の `input[type=file] capture=environment` を開く（切り抜き完了を待たない）。今の写真は裏で処理して行に積む。OS キャンセルでループ終了。ライブラリは `multiple` で複数枚を順に積む。追加ボタンの連打では二重取り込みや 20 行超を起こさない |
| 保存 | 「棚に並べる（N 本）」N = **保存対象行**の `count` 合計。行を上から順に `POST /api/bottles`（**1 行 = 1 リクエスト**。バッチ API は作らない）。同じ行の再送は同じ `operationKey`。全部成功で `/cellar` へ `replace` + トースト「棚に N 本並べました」+ 最初に作った本の段に M-32。一部失敗は成功行だけ消し、失敗行は理由と入力を残して再送できる。成功行は再送しない。写真のアップロードだけ失敗した行も保存対象に数え、押下時に写真を送り直してから保存する。残った行（送り直しても送れない・処理中・品名が空）は上部に理由をまとめて出す。アップロードの一時的な失敗（通信断・5xx）は自動で 2 回まで送り直す |
| 無効条件 | 保存対象行が 0 / 送信中。品名が空・処理中・写真失敗の行は保存対象に含めない（他の完了行の保存は止めない）。**読み取り中は無効にしない** |
| 中断 | 行削除・差し替え・全破棄・画面離脱後の遅延結果は行に反映しない。削除済みの行を完了コールバックで再生成しない。破棄後に届いた未紐付け写真は `DELETE` / 24h GC。Bitmap / Canvas / Blob URL は使い終わったら解放する |
- 失敗: 該当行に短い理由を残す。通知（トースト）だけにしない。フォーム上部にも残件数を出す。入力は保持
- 戻る（未保存）: 入力を触っていれば確認「入力を破棄しますか」。破棄時、未紐付け写真は `DELETE /api/photos/:id`
- 戻り先: 履歴。無ければ `/cellar`
- キャラクターは出さない（撮影タイルにも置かない）
- 調査用ログは行 ID・工程・エラーコード・HTTP ステータス・経過時間・バイト数・寸法・MIME だけ。画像本体・Base64・認証・位置情報・ファイル名・AI 生レスポンスは出さない
...
| # | 項目 | 内容 | データ |
|---|---|---|---|
| T0 | セラー名・品名 | セラー名（非操作・共有時だけアイコン）→ 品名見出し（**写真より上**。全文折り返す） | `cellarId`, `name` |
| T0s | Googleで調べる | 品名見出しの直下。表示中のボトル。棚・一覧には出さない。[drink-search.md](drink-search.md) | `name`, `producer`, `vintage`, `drinkType` |
| T0c | 種類・年・状態 | 種類 ・ 年（あるものだけ）と状態テキスト。棚: 「未開栓」。味わい中: 「味わい中（開栓日に開栓）」。貯蔵庫: 「飲み切り（飲み切り日）」+ 補助行「開栓日に開栓」。色だけで状態を出さない | `drinkType`, `vintage`, `status`, `openedOn`, `finishedOn` |
| T1 | 写真 | 中央 contain。高さ 220〜260px。棚板・傍線なし。無ければボトル型 SVG +「写真なし」。タップで全画面（ピンチ可）。閉じると元位置・スクロール復帰。**写真自体は動かさない** | `photos[0]`（表面） |
| T1b | 裏ラベル | 表面の脇の小サムネ（64×96、角 8px）+ ラベル「裏ラベル」。無ければ出さない。狭い画面は下のサムネ列。タップで全画面 | `photos[1]` |
| T3 | 開栓する | 棚のときだけ Button 主。**下部タブ直上に固定**。確認なし。即時 `POST open`。送信中「更新中…」+ 水位線（M-04）。連打防止。2xx で詳細に留まり状態が味わい中になり、任意の案内シートを出す。記録・ノートは自動作成しない | `POST /api/bottles/:id/open` |
| T9 | 飲み切った | 味わい中のとき「飲んだ量を記録」の右。確認なしで `finish`。トースト「飲み切りにしました」+「取り消す」 | `POST /api/bottles/:id/finish` |
| T10 | 開栓を取り消す | 味わい中で自分の記録が 0 件のときだけのテキストリンク。`restore` で未開栓へ。トースト「開栓を取り消しました」 | `POST /api/bottles/:id/restore` |
| T4 | 味わい中に戻す | 貯蔵庫のとき「飲んだ量を記録」の右。Button 副。即時 `reopen`。トースト「味わい中に戻しました」。旧「開栓の記録を取り消す」は置かない | `POST /api/bottles/:id/reopen` |
| T5 | 基本情報 | 見出しは「ボトル情報」「購入・保管」「メモ」の3区分（旧「基本情報」を分割）。ボトル情報: 品種 / 生産国 / 生産者。購入・保管: 購入日 / 価格 / 購入場所 / 保管日 / 保管場所 / 最終更新（既存の最終更新も含む）。上の品名・種類・年号・状態は繰り返さない。通常項目は項目名｜値の2列で値の左端を揃えて左寄せ。未入力（null・空文字）は「未登録」（未入力年を「NV」と書かない。価格0は¥0。明示の「不明」は維持）。日付は「2026年9月5日」。価格は `¥3,800`（整数・桁区切り）。長い値は右列で折り返して全文表示。メモは独立ブロックで改行保持の左寄せ。空は「メモはありません」。共有時は「参加者に共有」を補足 | data-model 6.3 |
| T6 | ノート節 | 最新 3 件 +「すべて（N）›」。作成導線は置かない | `GET /api/tasting-notes?bottleId=&limit=3` |
| T7 | 記録節 | このボトルの記録 最新 3 件。行 → `log-edit`。0 件なら節ごと出さない | `GET /api/drink-logs?bottleId=&limit=3` |
| T8 | 編集 | ヘッダー右 → `/cellar/:bottleId/edit` | — |

味わい中のとき:

- T3 は出さない。T3c「飲んだ量を記録」（主）と T9「飲み切った」を下部に横並び。記録 0 件なら T10

貯蔵庫（飲み切り）のとき:

- T3 は出さない。T3c「飲んだ量を記録」と T4「味わい中に戻す」を下部に横並び
...
| 開栓成功 | 詳細に留まり状態が味わい中になる。案内シート（T3b） |
...
| 場面 | 規則 |
|---|---|
| `log-new` の「ボトル」行 | **4-02 で表示する**（Phase 3 中は非表示。[drink-log.md](drink-log.md) 3.8）。ピッカーは `GET /api/bottles?view=all&q=`（貯蔵庫も含む）と `view=opened`。味わい中 → セラー → 貯蔵庫の順 |
...
4.1 と同じフィールドを任意で送る（送ったものだけ更新）。`count` は受け取らない。`status` / `consumedAt` / `consumedOn` / `sortOrder` も受け取らない（並びは `PUT /api/bottles/order`）。`drinkType` を変えたら新しい種類の先頭へ置く。空オブジェクトは 400。`photoIds` は **全体の差し替え**（`[表面, 裏面?]`。すでに紐付いている自分の写真 id はそのまま残し、新しい id は未紐付けから紐付け、配列に無い写真は R2 も削除）。残す写真も含めて添字を `sort_order` に書き直す。**送らない場合は写真を変えない。空配列 `[]` は写真をすべて外して削除する。**
...
| タイムアウト | プロファイルに従う（Gemini 25 秒 / Llama 20 秒）。表 + 裏の 2 枚は別枠（3.7 Flash 40 秒。出力上限も 4096 に上げる。[ai-recognition.md](ai-recognition.md) §3 / §8） | 502 `upstream_error`。画面は失敗帯 |
| 出力 | モデル出力は信頼しない入力として Zod 検証。落ちたフィールドは省く。`fields` 空でも 200。空の結果はキャッシュしない（押し直せば上流を呼び直す） | — |
...
プロンプトはサーバー固定。ユーザー入力（品名など）をプロンプトに含めない。画像も結果も保存しない。ログは件数・所要時間・成否・画像枚数・失敗理由（短い要約）・終了理由・トークン数のみ（値は出さない。[ai-recognition.md](ai-recognition.md) §10）。
...
| 裏面の加工 | 編集画面なし。端末内で瓶マスク（背景除去と同じモデル）の内側からラベルの矩形を探して切り出す（明るい紙に加えて印刷＝文字・罫線のある面もラベルとみなすので、暗い地に白文字のラベルも取れる。輸入者シールと本ラベルのように複数枚あれば全部を 1 枚に含める） → JPEG（長辺 1280・品質 0.82。縦横比はラベルなり）。ラベルが分けられなければ瓶の外接矩形、瓶が見つからない・切り抜き OFF・非対応なら 2:3 中央トリミング。背景除去（透過）・キャラ合成なし。常に `kind = photo` |
| 比率 | 2:3 |
...
| E47b | PATCH で `photoIds` 省略（名前だけ変更） | 既存写真は残る。空配列 `[]` のときだけ全解除 |
...
| 4-02 | ノート節（T6） | **5-04 で出す**。0 件でも見出しは出す（「書く ›」は置かない）。記録節は 4-02 で出す（0 件なら非表示） | 5-04 / T7 |
| 4-03 | 貯蔵庫の「飲んだ量を記録」 | **出す**（着地は `/logs/new?bottleId=`。ノート専用ボタンは置かない） | 記録へ統合 |
...
| 4-04 | 表示切替の記憶 | URL `?view=type\|list` + `localStorage` `cellar.listView`。既定は棚。「1 本ずつ」は廃止（`?view=one` は管理一覧に読み替え） | 04-cellar C4 |
| 4-04 | ヘッダー本数 | フィルタ前の `totalCount` | C2 |
| 4-04 | 貯蔵庫の写真 | 減彩しない（原色のまま。`filter: none`） | 04-cellar A3 |
...
| 裏面 | 読み取りの開始 | 新規は表面のあと B1c で裏ラベルの有無を聞き、答えが出たら自動（はい = 表 + 裏、いいえ = 表面だけ。2026-09-23 オーナー決定）。編集は表面の「使う」だけ自動。それ以外の裏面追加後は「裏面も含めて読み取る」（新規の成功帯、編集で保存済み表面だけのときは案内帯。失敗帯は「再読み取り」） | 先に表面を撮る導線。裏面を付けるたびに回数を消費しない。あとから裏面を足しても手動で表+裏を頼める |
| 裏面 | 裏面の加工 | 編集画面を開かない。ラベル部分だけを自動で切り出す（2026-09-23 オーナー決定。旧: 中央トリミングのみ） | 裏面は読み取り・確認用。ラベル以外を写さないほうが見やすく、読み取りにもラベルの文字を大きく渡せる。撮影の手数は増やさない |
