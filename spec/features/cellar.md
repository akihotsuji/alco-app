...
| B1b | 裏面（任意） | 「裏面を撮る」「裏面を選ぶ」（B1 の下。表面があるときだけ出す） | なし | 最大 1 枚。**編集画面（切り抜き・トリミング）は開かず**、2:3 中央トリミングの JPEG（`kind = photo`）にして即 未紐付け `POST /api/photos`。裏面だけの登録はできない（表面を削除したら裏面も外す）。サムネ + 「削除」。読み取り（B2）の材料にする。棚・一覧には出さない |
| B2 | ラベル読み取り帯 | 自動 + 手動 | — | 表面の「使う」で切り抜く前の 2:3 JPEG ができた時点で `POST /api/bottles/recognize`（背景除去・アップロードを待たない。同じ JPEG への要求は 1 リクエストにまとめる）。裏面を追加 / 差し替え / 削除したら **表 + 裏で再リクエスト**（結果は空欄と AI 印の欄だけを更新。触った欄は守る）。失敗帯には「再読み取り」リンクを出し、タップで同じ画像（表 + 裏）で再リクエストする（回数は加算される）。設定 `cellar.recognize` が OFF なら帯もリクエストも出さない |
...
| 裏面 | 行ごとに任意で **最大 1 枚**（G2b「裏面」）。編集画面は開かず 2:3 中央トリミングの JPEG（`kind = photo`）で即アップロード。追加・差し替え・削除で表 + 裏の再読み取り。行を外す × と破棄で表面と一緒に `DELETE` |
| ラベル読み取り | 行ごとに `POST /api/bottles/recognize`（切り抜く前の 2:3 JPEG。裏面があれば `back` パートも）。結果はその行の空欄、または直前の AI 印の欄に入れて AI 印。設定 `cellar.recognize` OFF なら呼ばない。1 日 10000 回（許容上限 MAX）の上限はそのまま（超えた行は失敗帯、保存は止めない）。**同時に走らせる読み取りは 2 行まで**（残りは待ち行列。上流の混雑による失敗を減らす） |
| 再読み取り | 失敗帯（上限・502・タイムアウト・`fields` 空）に「再読み取り」リンク。タップした**その行だけ**を同じ画像で再リクエストする（回数は加算される）。読み取り中は無効。成功帯にも「もう一度読み取る」を出さない（撮り直しで足りる） |
...
6. 裏面を追加 / 差し替え / 削除したら、表面の JPEG + 裏面の JPEG（あれば）で再リクエストする。古い要求の結果が後から返っても無視する（最新の要求だけ反映）
7. 「再読み取り」は同じ画像で再リクエストする。クライアントの同一 JPEG まとめ（B2）は再読み取りのときだけ解除する
...
| E42 | 裏面を追加したら読み取り結果が変わった | AI 印の欄だけ更新。ユーザーが触った欄は守る |
...
| 再読み取り | 対象 | 失敗帯からのみ。行単位。同じ画像 | 「読めなかった行だけやり直す」が目的。成功行の読み直しは撮り直しで足りる |
