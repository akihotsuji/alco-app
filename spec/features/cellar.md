...
| B6 | 詳細 | 折りたたみ | 閉じる | 生産者・産地（≦100）、ビンテージ（1800〜2100 / 空 = NV）、購入日、価格（整数円）、購入場所・保管場所（≦100）、メモ（≦2000）。ラベルは入力の左に置き間隔を開けない。AI が **生産者 / 産地 / ビンテージ**のいずれかを入れたら自動で開く |
...
| T5 | プロパティ | 順: 銘柄名 / 種類 / ビンテージ（無ければ「NV」）/ 産地 / 生産者 / 購入日 / 価格 / 購入場所 / 保管場所 / メモ。銘柄名・種類・ビンテージ以外は空行を出さない。**品種列は持たない**。価格は `¥3,800`（整数・桁区切り） | data-model 6.3 |
