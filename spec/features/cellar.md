...
| T1 | 写真 | 中央 contain。高さ 220〜260px + 短いガラス棚板。無ければボトル型 SVG +「写真なし」。タップで全画面（ピンチ可）。閉じると元位置・スクロール復帰。**写真自体は動かさない** | `photos[0]`（表面） |
...
| T5 | 基本情報 | 見出し「基本情報」。順: 品種 / 生産国 / 生産者 / 購入日 / 価格 / 購入場所 / 保管日 / 保管場所 / メモ（既存の最終更新も含む）。上の品名・種類・年号・状態は繰り返さない。空欄は「不明」（未入力年を「NV」と書かない）。日付は「2026年9月5日」。価格は `¥3,800`（整数・桁区切り）。長い値は折り返し。メモは独立ブロックで左寄せ | data-model 6.3 |
