...
| C5 | 検索 | Chip → Input。銘柄名・生産者・品種の部分一致。300ms デバウンス。最大 100 文字 | `q` |
...
| B6 | ボトル情報 | 常時 | 銘柄・種類の下 | **ビンテージ / 品種 / 生産者 / 産地**を折りたたまず出す。AI が空欄へ入れる（確度 0.5 未満は捨てる。触った欄は上書きしない） |
| B6b | 詳細 | 折りたたみ | 閉じる | **保管情報**（保管日・保管場所）→ **購入情報**（購入日・購入価格・購入場所）→ **メモ**。保管日の初期値は登録日（JST、詳細を閉じたままでも保存。未変更なら保存日の当日）。保管場所の初期値は「自宅セラー」。購入日は自動設定しない。編集では保存済みの保管日・保管場所を出し、空欄を補完しない |
...
| 項目 | 規則 |
|---|---|
| 行 | 1 枚の写真 = 1 行 = 1 銘柄。本数 N 可（1〜12）。上限 **20 行**。行の項目は銘柄名 / 種類 / 本数 / 生産者 / 産地 / ビンテージ / 品種（購入日・価格・場所・メモは持たない。あとで `bottle-edit`） |
...
| `storedOn` | Asia/Tokyo の `YYYY-MM-DD`。未来は不可。`purchasedOn` とは別列。新規作成で省略時はサーバーの JST 当日。ユーザーが変えた値は維持。フォームを開いたまま日付をまたぎ、保管日が初期値のままなら保存日の当日。`createdAt` は使わない |
...
| `vintage` | 整数 1800〜2100、または省略 / null（NV） | 「1800以上2100以下のビンテージを入力してください」 |
...
| `q` | 最大 100 文字。空は未指定。銘柄名・生産者・品種の部分一致（OR） | 「100文字以内で入力してください」 |
...
| XSS | 銘柄・生産者・産地・品種・店・保管場所・メモはテキスト。`dangerouslySetInnerHTML` 禁止 |
...
| 4-01 | 検索 | 銘柄名・生産者・品種の部分一致。`LIKE` + `%` `_` エスケープ。ASCII のみ大小無視 | 4.4 / data-model 7 章 |
...
| 本仕様 | recognize の `abvPercent` | API は返す。**フォームに入れない**。単体フォームのビンテージ / 品種 / 生産者 / 産地は常時表示（AI で詳細は開かない）。バッチは詳細折りたたみに品種を含め、推定が入ったら開く | ボトルに度数列が無い（要件 1.3 / data-model 6.3）。画面設計 B6 を同じ PR で修正 |
