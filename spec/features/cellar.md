...
| B1b | 裏ラベル（任意） | 「＋ 裏ラベルを追加（任意）」（表面があるときだけ出す） | なし | 最大 1 枚。**編集画面（切り抜き・トリミング）は開かず**、2:3 中央トリミングの JPEG（`kind = photo`）にして即 未紐付け `POST /api/photos`。裏面だけの登録はできない（表面を削除したら裏面も外す）。サムネ + 「裏ラベル」+ 変更／削除。読み取りの材料（手動の再判定で使う）。追加だけでは B2 を自動再実行しない。棚・一覧には出さない |
...
| 裏面 | 行ごとに任意で **最大 1 枚**（G2b「裏面」）。編集画面は開かず 2:3 中央トリミングの JPEG（`kind = photo`）でアップロード（同時 2 件枠を共有）。追加だけでは自動再読み取りしない。行を外す × と破棄で表面と一緒に `DELETE` |
...
| 保存 | 「棚に並べる（N 本）」N = **保存対象行**の `count` 合計。行を上から順に `POST /api/bottles`（**1 行 = 1 リクエスト**。バッチ API は作らない）。同じ行の再送は同じ `operationKey`。全部成功で `/cellar` へ `replace` + トースト「棚に N 本並べました」+ 最初に作った本の段に M-32。一部失敗は成功行だけ消し、失敗行は理由と入力を残して再送できる。成功行は再送しない |
