...
| A3 | ボトル | 棚と同じ切り抜き。**減彩しない**（原色のまま。`filter: none`）銘柄最大 2 行 + 年号。右上に「開栓 9/5」ピル（`consumedOn`。開栓日。飲み終わり日と誤記しない） | `items[]` |
...
- 説明: 「開栓したボトルが日付順に並びます。未開栓のボトルはセラーで管理します。開栓と飲み終わりは同じ状態です。」
...
| T0c | 種類・年・状態 | 種類 ・ 年（あるものだけ）と状態テキスト。棚: 「未開栓」。貯蔵庫: 「開栓（長い日本語日付）」。色だけで状態を出さない | `drinkType`, `vintage`, `status`, `consumedOn` |
...
| T3 | 開栓する | 棚のときだけ Button 主。**下部タブ直上に固定**。確認なし。即時 `POST consume`。送信中「更新中…」+ 水位線（M-04）。連打防止。2xx で詳細に留まり、任意の案内シートを出す。記録・ノートは自動作成しない | `POST /api/bottles/:id/consume` |
| T4 | 開栓の記録を取り消す | 貯蔵庫のとき主操作エリア。Button 副。即時 `restore`（`consumed → sealed`。飲酒記録は残す）→ **棚の詳細**へ。送信中「更新中…」。トースト「開栓の記録を取り消しました」（`cheer` + M-25） | `POST /api/bottles/:id/restore` |
...
- T3 は出さない。T3c「飲んだ量を記録」と T4「開栓の記録を取り消す」を下部の主操作エリアに出す
