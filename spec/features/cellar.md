...
- 状態: **レビュー待ち**（オーナー承認後に 4-02 以降へ着手する。10 章の決定事項を含む）
...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| B1 | 写真タイル | `photo-edit`（2:3、`cellar`、切り抜き既定 ON、**キャラ合成なし**） | なし | 1 枚。任意。「使う」直後に未紐付け `POST /api/photos`。`?camera=1` ならマウント直後に開く（× で閉じてもフォームは残る） |
| B2 | ラベル読み取り帯 | 自動 | — | 写真確定直後に切り抜く前の 2:3 JPEG を `POST /api/bottles/recognize`。設定 `cellar.recognize` が OFF なら帯もリクエストも出さない |
...
| 4-07 | モデル名 | 本仕様では決めない。4-07 で定数化 | api-design 4.5.3 |
...
| 4-05 テスト総仕上げ | 4 / 8 / 9 | 各受け入れ。抜けは本タスク |
