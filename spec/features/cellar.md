...
| 表示切替 | クライアントの棚レイアウト。`?view=one\|type\|list`（API の `view=cellar\|archive\|all` とは別） |
...
### 3.1 `bottle-list` セラー（`/cellar?view=one|type|list&q=&drinkType=`）
...
| C4 | 表示切替 | 「棚」「1本ずつ」「管理一覧」。URL `?view=` と `localStorage` `cellar.listView`（既定 `one`）。1本ずつを「一覧」と呼ばない | クライアント |
| C4b | 管理一覧 | 1 行 = 1 本。同名統合なし。写真・銘柄・年号・国・本数 | `items[]` |
| C5 | 検索 | Input（常時）。品名・生産者・品種の部分一致。300ms デバウンス。最大 100 文字 | `q` |
...
| A3 | ボトル | 棚と同じ切り抜き。CSS `filter: saturate(0.5) brightness(0.9)`（要素表が正。モックの数値は使わない）。銘柄最大 2 行 + 年号。右上に「開栓 9/5」ピル（`consumedOn`。開栓日。飲み終わり日と誤記しない） | `items[]` |
...
| 再読み取り | 失敗帯（上限・502・タイムアウト・`fields` 空）に「再読み取り」リンク。成功帯かつ裏面があるときは「裏面も含めて読み取る」。タップした**その行だけ**を表 + 裏（裏が無ければ表だけ）で再リクエストする（回数は加算される）。読み取り中は無効。成功帯に汎用の「もう一度読み取る」は出さない。AI 失敗でも写真と手入力は残し、保存を止めない |
...
| 4-04 | 表示切替の記憶 | URL `?view=one\|type\|list` + `localStorage` `cellar.listView`。既定 1 本ずつ | 04-cellar C4 |
