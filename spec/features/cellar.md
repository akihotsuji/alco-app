...
| 裏面の加工 | 編集画面なし。端末内で瓶マスク（背景除去と同じモデル）の内側からラベルの矩形を探して切り出す → JPEG（長辺 1280・品質 0.82。縦横比はラベルなり）。ラベルが分けられなければ瓶の外接矩形、瓶が見つからない・切り抜き OFF・非対応なら 2:3 中央トリミング。背景除去（透過）・キャラ合成なし。常に `kind = photo` |
| 比率 | 2:3 |
