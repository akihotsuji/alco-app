...
| 写真の付け替え | 表面: 「削除」→ 撮り直し（未紐付け）→ 保存で `PATCH { photoIds: [新 id, 裏面 id?] }`。裏面: 「削除」/「裏面を撮る」で同様。`photoIds` は常に **[表面, 裏面?]** の全体を送る（外れた写真はサーバーが R2 ごと削除）。保存前に外した保存済み写真は `DELETE` しない（キャンセルで戻せる） |
