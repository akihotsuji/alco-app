...
未読数（`read_at` が空の本人宛て通知）はヘッダー F2 のバッジに加え、ホーム画面に追加したアプリのアイコンにもバッジで出す（[pwa.md](pwa.md) 6.2）。件数だけで本文は出さない。アプリが動いている間だけ更新し、閉じている間は古い件数が残ることがある。プッシュは送らず、通知の許可も求めない。ログアウト・アカウント削除で消す。
