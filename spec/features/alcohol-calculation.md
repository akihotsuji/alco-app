...
| `abv_percent` | **0〜100**、小数第 1 位まで。**0% は可** | ノンアルコールも記録できる。0g の行があっても休肝日にはしない |
| ボトル丸ごと | **記録できる**。専用画面は作らない。量の手入力とボトル量チップ | 原則はグラス。375 / 750 / 1500ml を全種類で選べる |
| 量チップ | 種類ごとの下表。常に「手入力」あり | 変えるときだけ使う（1-02） |
| 種類変更 | その種類のデフォルト量・度数で **上書き** | 最短タップ。編集中の手入力は捨てる |
| 初期種類 | **赤ワイン**（125ml / 12%） | 1-02。種類 → 保存の 2 タップ。ワイン系は同じプリセット |
...
|---|---|---:|---|---:|---|
| `wine_red` | 赤ワイン | 125 | 125 / 150 | 12 | グラス1杯（新規既定） |
| `wine_white` | 白ワイン | 125 | 125 / 150 | 12 | グラス1杯 |
| `wine_rose` | ロゼ | 125 | 125 / 150 | 12 | グラス1杯 |
| `wine_sparkling` | スパークリング | 125 | 125 / 150 | 12 | グラス1杯 |
| `wine_orange` | オレンジ | 125 | 125 / 150 | 12 | グラス1杯 |
| `wine` | ワイン | 125 | 125 / 150 | 12 | 色不明・既存データ |
...
- **ボトル量チップ**（全種類共通）: **375 / 750 / 1500**。グラス量チップと並べて出す。専用画面は作らない
- チップに無い量（一升 1800、マグナム以外など）は **手入力**
- 焼酎 60ml は原酒目安。ロック多めは記録時に 90ml へ直せばよい
- 「その他」は量・度数が空。両方入るまで保存不可（1-02）
- `log-new` を開いた初期選択は赤ワイン（125 / 12）。ユーザーはすぐ保存できる
...
export const DRINK_TYPE_PRESETS = {
  wine_red: { volumeMl: 125, abvPercent: 12, volumeChips: [125, 150] },
  wine_white: { volumeMl: 125, abvPercent: 12, volumeChips: [125, 150] },
  wine_rose: { volumeMl: 125, abvPercent: 12, volumeChips: [125, 150] },
  wine_sparkling: { volumeMl: 125, abvPercent: 12, volumeChips: [125, 150] },
  wine_orange: { volumeMl: 125, abvPercent: 12, volumeChips: [125, 150] },
...
| `abvPercent` | 0〜100、小数第 1 位 | **0 は可**。100 は可 |
...
- `z.number().multipleOf(0.1)` は IEEE で失敗しうる。`min(0).max(100)` + refine（第 1 位）にする
...
| 0g 記録 | 休肝にしない（0% や 1ml × 0.1% で保存 0.00g でも 1 件） |
| 今日 | 集計上は記録 0 なら休肝日。ホームの今日カードは **未記録** と出し、杯数 0 だけで「休肝」とは表示しない（明示的な休肝日登録は未実装） |
...
| 1ml 0.1% | 0.0008 | 0.00 | 0.0 |
| 350ml 0%（下限度数） | 0 | 0.00 | 0.0 |
| 750ml 12%（ボトル） | 72 | 72.00 | 72.0 |
| 5000ml 100%（上限） | 4000 | 4000.00 | 4000.0 |

1ml × 0.1% も 350ml × 0% も表示 0.0g でも **記録 1 件**。その日は休肝日ではない。
...
- [x] 12 種類すべてにデフォルト方針がある（その他は手入力。ワイン系は 125 / 12）
- [x] 例題の期待値が書いてある
- [x] 丸めと日付境界が書いてある
- [x] オーナー承認（2026-09-04）
