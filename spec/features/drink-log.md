...
| 写真付き | ホームの「写真から記録」→ 使う → 保存 = 3 タップ（OS の撮影操作を除く）。入力画面の「写真を撮る」からも同じ |
...
| 入口 | 開き方 | 写真 |
|---|---|---|
| 中央タブ「記録」 / ホームの「お酒を記録する」 | `/logs/new` | なし。撮影は開始しない |
| ホームの「写真から記録」 | `photo-edit` で撮影 → 「使う」の **後に** 本画面を開く。× / OS キャンセルでは本画面を開かない | あり。N2 のサムネ。撮影結果は共有状態から受け取り、URL には載せない |
...
| H9 | 写真から記録 | 記録用 `photo-edit` をその場で開き、「使う」で `/logs/new` を写真付き（N2）で開く。× / OS キャンセルはホームに留まる。`?camera=1` は使わない。中央タブは撮影しない | — |
