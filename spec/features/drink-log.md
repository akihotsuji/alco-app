...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」 | なし | 1 枚のみ。明示タップ以外では起動しない（`?camera=1` 含む）。「使う」直後に未紐付けで `POST /api/photos` → `photoId` を保持。キャラ合成**前**の 4:5 JPEG で `POST /api/drink-logs/recognize`（種類・量・任意で度数。確度 0.5 未満は捨てる。ユーザーが先に触った欄は上書きしない。ボトル選択中は種類を変えない）。失敗・上限は黙って初期値のまま |
