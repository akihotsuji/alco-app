...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」 | なし | **フォーム先頭**。1 枚のみ。明示タップ以外では起動しない（`?camera=1` 含む）。「使う」直後に未紐付けで `POST /api/photos` → `photoId` を保持。キャラ合成**前**の 4:5 JPEG で `POST /api/drink-logs/recognize`（品名・識別 4 項目・種類・量・度数。確度 0.5 未満は捨てる。空欄と直前の AI 値は再読取で上書き。触った欄は上書きしない。ボトル選択中は種類を変えない。生産国・品種はラベル表記が無くても手がかりから推測）。撮影日を日時の既定にする（`?date=` の過去日は維持）。失敗・上限は黙って初期値のまま |
...
| N8b | 識別 | ヴィンテージ / 品種 / 生産者 / 生産国 | 空 | 任意。AI は空欄、または直前の AI 値を再読取で上書き。生産国・品種はラベル情報から推測。ボトル選択時は空欄ならボトルから |
