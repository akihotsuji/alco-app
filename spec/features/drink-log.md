...
| H2〜H5 | 今日カード | 杯数 `totalCount`、g `displayAlcoholGrams(totalAlcoholG)`、0 件なら「休肝」ピル。**タップで今日の `/logs`（`log-day`）**。週マス（H6）と「今週 ›」（H13）はカードのタップ判定から除く | `GET /api/drink-logs/summary?period=day&date=<今日>` |
...
| H8 | 記録する | `/logs/new`（写真なし。写真タイルは入力画面で任意） | — |
| H9 | カメラ | **中央タブと同じ**。記録用 `photo-edit` をその場で開き、「使う」で `/logs/new` を写真付き（N2）で開く。× / OS キャンセルはホームに留まる。`?camera=1` は使わない | — |
...
| H13 | 今週 › | 今日カード右上のテキストリンク → `summary-week` | — |
...
- 入口: 週サマリーはホームの今日カード内「今週 ›」（H13）と月サマリーの週行。月サマリーは週サマリーの「今月 ›」だけ。ホームの今日カード本体は週ではなく今日の `log-day` へ行く（2026-09-06 (c)）
...
| 本仕様（2026-09-06 追記） | 日別・サマリーの入口 | 日別 = ホームの今日カード / 週マス / 保存後 / 週サマリーの行。週 = 今日カード内「今週 ›」/ 月の週行。月 = 週の「今月 ›」 | [screen-designs/02-home.md](../screen-designs/02-home.md) H2 / H13 |
