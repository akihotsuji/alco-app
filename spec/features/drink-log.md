...
| N3 | 種類 | セレクト（全 7 種） | **ワイン**（写真があれば推測で先に選ぶ） | ユーザーが選ぶと量・度数を **その種類のデフォルトで上書き**（触った値は捨てる）。「その他」は両方空。既存の選択肢は削除しない。AI が種類を入れるときは量は **推測値を優先**し、無ければ種類デフォルト |
...
| N4 | 飲んだ量 | 数値 + ml + プリセット | 種類デフォルト（ワイン 125。125 を一律にしない） | 直接入力（整数 1〜5000）。近くに種類のよく使う量。残り（ボトル量 **375 / 750 / 1500**）は「その他」 |
...
| N10 | 記録を保存 | 固定バー | 有効（初期ワイン） | 無効条件: 量または度数が空 / 範囲外、日時が範囲外、写真アップロード中・失敗中、保存中。無効時は短い説明。未操作欄に初期エラーを並べない。二重送信防止。失敗時は入力保持 |
