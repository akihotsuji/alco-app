...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真タイル | `photo-edit`（4:5、プリセット `table`、キャラ合成トグルあり） | なし | 1 枚のみ。タップは撮影、下の「ライブラリから」は保存済み写真。「使う」直後に未紐付けで `POST /api/photos` → `photoId` を保持。`?camera=1`（ディープリンク）ならマウント直後に撮影から開く（× で閉じても本画面は残る） |
