...
| 2 タップ記録 | ホーム「記録する」→ 保存（`log-new` は初期値ワイン 125ml / 12% で即保存できる。写真なし） |
| 写真付き | 中央タブ（またはホームのカメラ）→ 使う → 保存 = 3 タップ（OS の撮影操作を除く）。入力画面の写真タイルからも同じ 3 タップ |
| 中央タブ | **撮影の開始**（着地画面なし）。× / OS キャンセルは何もしない。2026-09-06 (c) |
...
| セラー開栓時の記録自動作成 | 作らない（2026-09-06）。1 杯は本機能の `log-new` からボトルを選ぶ |
...
中央タブの着地ではない（2026-09-06 (c)）。親タブはホーム。入口は **ホームの今日カード（今日）/ ホームの週マス（その日。未来は無効）/ `log-new` `log-edit` の保存後（`?highlight=`）/ 週サマリーの日別行** の 4 つ。記録の入口（記録する / カメラ / マイドリンク）はホーム（3.5）に集約し、この画面には置かない。
...
| D3〜D5 | （廃止） | 「記録する」「カメラ」「マイドリンク」は日別に置かない（ホーム H8 / H9 / H11）。番号は欠番のまま残す。過去日への記録は `log-new` の N7 で日時を変える | — |
...
| D9 | 保存後トーストの取り消し | `log-new` から到着したときのトースト「記録しました  取り消す」（5 秒）。「取り消す」→ `DELETE /api/drink-logs/:id`。消える行は inset → フェードアウト（X2）、合計は再取得でカウント（M-09）、0 件になれば「休肝」ピル（M-18） | `DELETE /api/drink-logs/:id` |
...
- 空状態: 合計「休肝」+「この日の記録はまだありません」。キャラ・ボタンは出さない。全期間 0 件の初回だけ、一覧 API の `hasAnyLogs: false` を使って「中央の記録ボタンから写真を撮って記録できます」（X7）
...
入口（2026-09-06 (c)。フォームの項目・規則は変えない）:

| 入口 | 開き方 | 写真 |
|---|---|---|
| 中央タブ / ホームのカメラ | `photo-edit` で撮影 → 「使う」の **後に** 本画面を開く。× / OS カメラ・ピッカーのキャンセルでは本画面を開かず、タップ前の画面に留まる | あり。N2 のサムネ（アップロード進捗付き）で始まる。撮影結果は `photo-edit` の共有状態（`attachments.log`）から受け取り、URL には載せない |
| ホームの「記録する」 | `/logs/new` | なし（N1 の写真タイルは任意） |
| ディープリンク | `/logs/new?date=&camera=1&bottleId=` | `?camera=1` ならマウント直後に `photo-edit`（従来どおり。アプリ内の導線は使わない） |

| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真タイル | `photo-edit`（4:5、プリセット `table`、キャラ合成トグルあり） | なし | 1 枚のみ。タップは撮影、下の「ライブラリから」は保存済み写真。「使う」直後に未紐付けで `POST /api/photos` → `photoId` を保持。`?camera=1`（ディープリンク）ならマウント直後に撮影から開く（× で閉じても本画面は残る）。キャラ合成**前**の 4:5 JPEG で `POST /api/drink-logs/recognize`（種類・量・任意で度数。確度 0.5 未満は捨てる。ユーザーが先に触った欄は上書きしない。ボトル選択中は種類を変えない）。設定トグルは無く常に試す。失敗・上限は黙って初期値のまま |
| N2 | 撮影後サムネ | 96×120 + 「編集」「削除」 | — | 「編集」は元の Blob から再編集（旧 `photoId` は削除して新規アップロード）。「削除」は確認なしで即 `DELETE /api/photos/:id` |
| N3 | 種類 | Chip ×7（横スクロール） | **ワイン**（写真があれば推測で先に選ぶ） | ユーザーが選ぶと量・度数を **その種類のデフォルトで上書き**（触った値は捨てる）。「その他」は両方空。AI が種類を入れるときは量は **推測値を優先**し、無ければ種類デフォルト |
| N4 | 量 | スコア + 量チップ + 手入力 | 125（写真があれば推測値。チップにあればそのチップを選択） | チップ = 種類の量チップ + ボトル量 **375 / 750 / 1500**（全種類共通）+「手入力」。整数 1〜5000 |
...
| N8 | ボトル | 行「選ぶ ›」→ ボトルピッカー | `?bottleId=` があれば事前選択 | **4-02 で表示**。選ぶとボトルの種類・名前を反映（3.8） |
...
- 成功: 対象日（応答の `drunkOn`）の `log-day?highlight=<id>`（今日は `/logs?highlight=<id>`、他は `/logs/:date?highlight=<id>`）へ **即** `replace`（ボタン側の成功表示は無し。M-05）。トースト「記録しました  取り消す」（`cheer` 32px + 水面 M-25、5 秒）。保存中はボタンに水位線（M-04）、失敗で静かに戻す（M-06）
...
記録の入口（記録する / カメラ / マイドリンク）と、日別・週 / 月サマリーへの入口をこの画面に集約する（2026-09-06 (c)）。
...
| H6 | 今週 | 記録操作の下に独立。月〜日の 7 日（曜日・日付・小さな印）。記録ありと今日は別表示。**日付のタップでその日の `/logs/:date`**（X6。未来は無効）。今日が 0 → 記録ありになる瞬間に印が下から満ちる（M-17） | `GET /api/drink-logs/summary?period=week&date=<今日>` |
...
| H8 | お酒を記録する | `/logs/new`（写真なし。写真タイルは入力画面で任意） | — |
| H9 | 写真から記録 | **中央タブと同じ**。記録用 `photo-edit` をその場で開き、「使う」で `/logs/new` を写真付き（N2）で開く。× / OS キャンセルはホームに留まる。`?camera=1` は使わない | — |
...
- **楽観更新しない**。サーバー応答後に day / week サマリーを再取得して数字を更新する（旧値 → 新値のカウント M-09。未記録ピルは M-18 でフェード。チップは送信中 inset 維持 → 成功で水位線 M-07 / M-08。`haptic("success")` は設定 ON のみ）
...
| E8b | 中央タブ / ホームのカメラで撮影をキャンセル | × / 撮り直しの取りやめ / OS カメラ・ピッカーのキャンセルはすべて **何もしない**。タップ前の画面にそのまま留まり、`log-new` は開かない。写真は未アップロードなので後始末も無い |
| E8c | 中央タブで「使う」した後、`log-new` を保存せずに戻る | E22 と同じ（確認ダイアログ → 破棄で `DELETE /api/photos/:id`）。戻り先はタップ前の画面（履歴） |
...
| 本仕様（2026-09-06 追記） | 日別の主ボタン群 | `log-day` に「記録する」「カメラ」「マイドリンク」を **置かない**（ホーム H8 / H9 / H11 に集約）。日別は一覧・合計・編集・削除。過去日の 1 タップは無くなり、過去日の記録は `log-new` の日時行で行う | 同上。日別は着地ではなくなった |
