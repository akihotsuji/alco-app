...
毎日使うコア機能。**最短タップ数で 1 杯を記録できる**ことを最重要の UX 要件とし、写真・ボトル・メモ・テイスティングは任意の上乗せにする。
...
| D6 | 記録行 | 1 行目: 名前（`drinkName` があればそれ、無ければ種類の表示名）+ 「量ml」、右端に表示丸めの g。2 行目: 「HH:MM ・ 度数% ・ ★評価（`tastingNote` があれば）・ ボトル名（`bottleId` があれば）・ 場所名（`placeName` があればテキストのみ。行全体が Link なのでマップリンクは置かない）」。左にサムネ 48px（`thumbPhotoId` があれば `GET /api/photos/:id/content?variant=thumb`、無ければ種類アイコン） | `items[]` |
| D7 | 行タップ | `log-edit` | — |
...
| N2s | Googleで調べる | テキストリンク | — | 品名直下。有効な品名があるときだけ。日別行には出さない。[drink-search.md](drink-search.md) |
...
| N5 | アルコール度数 | 数値 + % + ステッパー（0.5 刻み。長押しで連続）+ 直接入力 | 12 | 0〜100、小数第 1 位。0 可。− / 数値 / % / ＋ を近接。格子外からの ± は進行方向の次の 0.5 へスナップ |
| N6 | ライブ g | テキスト | 純アルコール量 12.0 g | `displayAlcoholGrams(calculateAlcoholGrams(volume, abv))`。量または度数が空なら「—」。保存値の丸めと一致する |
| N7 | 飲んだ日時 | ラベル左 + 行 → ネイティブ `datetime-local` | 今日: 「今日 HH:MM」（いま）。`?date=` あり: 「9月4日 20:00」 | JST 固定で解釈（3.7）。現在 +15 分まで。写真があれば撮影日（ユーザー未操作時） |
...
| N11 | テイスティングを残す | 折りたたみ | 閉じる | 閉じたままなら `tastingNote` を送らない。開いたときだけ `ratingX10` 必須。4 欄とノート写真 ≦6 は任意。識別は記録側 |
| N10 | 記録を保存 | 固定バー | 有効（初期赤ワイン） | 無効条件: 量または度数が空 / 範囲外、日時が範囲外、写真アップロード中・失敗中、テイスティングを開いて評価未選択、保存中。無効時は短い説明。未操作欄に初期エラーを並べない。二重送信防止。失敗時は入力保持 |

- 初期状態（ワイン 125 / 12、日時いま、写真なし、テイスティング閉じ）で **すぐ保存できる**（保存 1 タップ）
- 成功: 対象日（応答の `drunkOn`）の `log-day?highlight=<id>`（今日は `/logs?highlight=<id>`、他は `/logs/:date?highlight=<id>`）へ **即** `replace`（ボタン側の成功表示は無し。M-05）。保存後ダイアログと `fromLog` は出さない。トースト「記録しました  取り消す」（`cheer` 32px + 水面 M-25、5 秒）。保存中はボタンに水位線（M-04）、失敗で静かに戻す（M-06）
...
| テイスティング | 既存ノートがあれば節を開く。無ければ閉じる。開いて保存すると upsert。`tastingNote: null` でノートだけ削除 |
...
| `tastingNote` | 任意。オブジェクトなら `ratingX10` 必須、4 欄任意、`photoIds` ≦6。PATCH の `null` は子ノート削除。識別・日付は受け取らない | 「評価を選んでください」 |
...
| `log-new` 保存 | `POST /api/drink-logs`（`photoIds`, `bottleId`, `myDrinkId`, 任意 `tastingNote`） | 3-02 / 記録へ統合 |
