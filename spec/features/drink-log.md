...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」 | なし | **フォーム先頭**。1 枚のみ。明示タップ以外では起動しない（`?camera=1` 含む）。「使う」直後に未紐付けで `POST /api/photos` → `photoId` を保持。キャラ合成**前**の 4:5 JPEG で `POST /api/drink-logs/recognize`（種類・量・任意で度数。確度 0.5 未満は捨てる。ユーザーが先に触った欄は上書きしない。ボトル選択中は種類を変えない）。失敗・上限は黙って初期値のまま |
...
| N7 | 飲んだ日時 | ラベル上 + 行 → ネイティブ `datetime-local` | 今日: 「今日 HH:MM」（いま）。`?date=` あり: 「9月4日 20:00」 | JST 固定で解釈（3.7）。現在 +15 分まで |
| N8 | ボトル | 行「選ぶ ›」→ ボトルピッカー | `?bottleId=` があれば事前選択 | **4-02 で表示**。選ぶとボトルの種類・名前を反映（3.8） |
| N9 | メモ（任意） | Textarea、残数表示 | 空 | 0〜500 文字。前後空白を除いて空なら `null`。プレースホルダ「飲んだ場所など」 |
