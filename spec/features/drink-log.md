...
- 成功: ダイアログ「テイスティングノートをつける？」。「つける」→ `/notes/new?fromLog=<id>` へ `replace`（写真はコピーしない）。「あとで」/ 閉じる → 対象日（応答の `drunkOn`）の `log-day?highlight=<id>`（今日は `/logs?highlight=<id>`、他は `/logs/:date?highlight=<id>`）へ **即** `replace`（ボタン側の成功表示は無し。M-05）。トースト「記録しました  取り消す」（`cheer` 32px + 水面 M-25、5 秒）。保存中はボタンに水位線（M-04）、失敗で静かに戻す（M-06）
