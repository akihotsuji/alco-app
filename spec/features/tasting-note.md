...
| スナップショット | `drink_name` / `drink_type`。ボトル選択時にサーバーがボトルからコピーし、以降ボトルを改名してもノートは当時の値を保持する。`vintage` はノート独自（サーバーはボトルから上書きしない。クライアントが空欄ならボトルの年を埋める） |
| 都度入力 | `bottle_id` が NULL。品名・種類を手入力する |
| 評価（`ratingX10`） | 総合評価。UI は 1.0〜5.0 の 0.1 刻み。保存・API は整数 **10〜50・1 刻み**。表示は `/ 10` |
...
| L3 | 評価 | Chip「★4 以上」。トグル。ON で `ratingX10Min=40`。OFF でクエリを外す。選択中は `aria-pressed` と解除用 aria-label。4.0 未満を除外。`ratingX10Max` は UI に置かない | `ratingX10Min` |
...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」+ サムネ 96×120 × 最大 6 | なし | **フォーム先頭**。「写真を撮る」「写真を選ぶ」→ OS 確定の直後に **`photo-edit` を開かず**中央 4:5 で自動トリミングし、設定どおりキャラ合成して行へ積む（2026-09-10。「使う」の確認は挟まない）。位置を直すときだけサムネの「編集」で `photo-edit`。確定直後に未紐付け `POST /api/photos`。切り抜く前の 4:5 JPEG で `POST /api/tasting-notes/recognize`（品名・種類・識別 4 項目。確度 0.5 未満は捨てる。空欄と直前の AI 値は再読取で上書き。触った欄は上書きしない。ボトル選択中は種類を変えない。生産国・品種は根拠があるときだけ）。撮影日をテイスティング日の既定にする（`fromLog` と編集では維持）。**自動起動しない**（`?camera=1` も無視）。7 枚目は撮る／選ぶを無効化し「写真は 6 枚までです」 |
...
| — | 読み取り帯 | 自動 | — | 「写真から読み取り中…」→「写真から入れました」/「読み取れませんでした」。設定 OFF は置かない（セラーの `cellar.recognize` とは別。ノートは常に試す。429 / 502 は失敗帯。保存は止めない） |
| N2 | セラーから選ぶ | 新規先頭の「セラーから選ぶ / 新しく入力」+ 見えるボタン | `?bottleId=` があれば事前選択 | 入力補助。未登録も下の欄で入力できる。空欄の品名・種類・識別だけ埋める（ユーザー修正は上書きしない）。選択済みは対象と変更・解除。共有ボトルに紐づいてもノートは個人。`?fromLog=` は記録の識別・日付を先に入れる |
| N3 | 品名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。品名 1〜100。種類は 12 種。AI は空欄または直前の AI 値（種類は未操作）のとき入れる |
| N3s | Googleで調べる | テキストリンク | — | 新規は品名直下。編集は折りたたみ閉じていても要約付近。開くと二重表示しない。[drink-search.md](drink-search.md) |
| N3b | お酒の情報 | 折りたたみ | 閉じる | 品種・生産者・生産国・ヴィンテージ。編集は要約 +「お酒の情報を編集」。検証エラー時だけ開く。ヴィンテージは 1800〜2100 / 空 = 未登録。ボトル選択時は空欄ならボトルから。AI は空欄、または直前の AI 値を再読取で上書き |
| N4 | テイスティング日 | ラベル上 + 行 → ネイティブ `date` | 今日（JST）。`fromLog` は記録の `drunkOn` | `YYYY-MM-DD`。今日は可。**未来不可**。端末 TZ に依存しない。写真があれば撮影日（ユーザー未操作時。`fromLog` / 編集は維持） |
| N5 | 評価 | 星 5 + スライダー + 左右 ± + 数値表示 | **未選択** | 1.0〜5.0、0.1 刻み。スライダー（`input type=range` step 0.1）が主で星が連動して埋まる。スライダー左右の − / ＋ で 0.1 増減（未選択からは 1.0）。星タップは整数。再タップしても未選択へ戻さない。キーボード・読み上げはスライダー。0 として保存しない。数値入力欄は置かない（2026-09-14） |
| N6 | ひとこと | Input（1 行） | 空 | **`taste` そのもの**。別列は作らない。任意 |
| N7 | 詳しく書く | 折りたたみ | 閉じる | 既存 4 欄は削除しない。開くと `taste` が複数行 Textarea に広がり、外観 / 香り / 余韻の Textarea が並ぶ。各 ≦2000。残数表示 |
| N8 | ノートを保存 | Button 主（固定バー） | 無効 | 無効: 評価未選択 / 品名空（ボトル未選択時） / 種類未選択（ボトル未選択時） / 未変更（編集） / 飲んだ日が不正・未来 / 写真アップロード中・失敗中 / 保存中。無効時「必須項目を入力してください」または「変更はありません」。未操作欄に初期エラーを並べない。削除は距離を取る |
...
| V1s | Googleで調べる | 本文の品名直下。表示中のノート。一覧カードには出さない。[drink-search.md](drink-search.md) | 表示中のノート |
| V2 | 評価 + 日付 | 星（塗り = `--primary`）+ 数値 +「2026年8月1日」。ヴィンテージがあれば続けて出す | `ratingX10`, `tastedOn`, `vintage` |
...
| フィールド | 規則 | エラー文 |
|---|---|---|
| `ratingX10` | 整数 10〜50、**1 の倍数**（UI の 0.1）。POST 必須 | 欠け・範囲外・刻み不正は「評価を選んでください」 |
...
| `producer` / `origin` / `variety` | 任意。各 ≦100。trim 後に空なら `null`。ボディがあれば採用、省略時は作成でボトルからコピー | 「100文字以内で入力してください」 |
...
| `ratingX10Min` / `ratingX10Max` | 10〜50、1 刻み。両方あるとき `min <= max` | 「評価の範囲が正しくありません」 |
...
クライアント: 確度 0.5 未満は捨てる。空欄と直前の AI 値は再読取で上書きする。ボトル選択中は種類を変えない。新規・編集の両方で使う。
...
| 未選択 | 送らない | 保存不可 |
| 0 / 0.5 | — | 不可 |
| 5.1 / 1.2（float） / 9 | — | 不可（400） |

- 表示は `ratingX10 / 10`。一覧・詳細は小数第 1 位まで出す（`4.0` / `4.2`）
- 星の塗り: `floor(ratingX10 / 10)` 個を `--primary`。余りは次の星を割合で塗る（4.3 なら 4 個 + 30%）
- スライダー: `input type=range` の 1.0〜5.0 / step 0.1 を `ratingX10FromSlider` で整数化（範囲外・0.1 未満刻みは捨てる）。塗りは `ratingSliderFillRatio`（可動域に対する 0〜1。未選択は 0）
- スライダー左右の − / ＋: `stepRatingX10`（±0.1、未選択からは 1.0）
- 星タップ: 整数 1〜5（`ratingX10 = 10 * n`）。すでにその整数のとき、タップし直しても 0 にはしない（未選択へ戻さない。クリアはしない）
- 共有ヘルパ（5-02 で `src/shared`）: `RATING_X10_MIN = 10` / `MAX = 50` / `STEP = 1`、`isValidRatingX10`、`formatRatingX10`（`"4.2"`）
- Zod は `.int().min(10).max(50).refine(n => n % 1 === 0)`。`.multipleOf(0.1)` を float に使わない
...
| 比率 | 4:5 |
| プリセット | 色補正なし |
| キャラ合成 | **あり**（設定 S3 の `photo.mascot` を自動適用）。右下。処理開始時に 4 ポーズから 1 つ抽選。背後グローは敷かない |
| 切り抜き（背景除去） | **しない**（セラーのみ） |
...
1. 撮影・選択の確定（再編集は `photo-edit` → 「使う」）→ 未紐付け `POST /api/photos`（`tastingNoteId` は送らない）
2. フォームが id を配列で保持
...
| E6 | 評価 5.1 / 1.2（float） / 0 / 9 | 400「評価を選んでください」。1.1（11）は可 |
| E7 | 飲んだ日が未来 | N8 無効 + 400 |
| E8 | 飲んだ日が不正（`2026-02-30`） | 400「日付の形式が正しくありません」 |
| E9 | ボトル未選択で品名空 | N8 無効。種類だけ選んでも保存不可 |
...
| E36 | `camera=1` や撮影キャンセル | フォームは残る。撮影は自動で始まらない。写真 0 枚のまま進められる |
| E37 | 読み取り中に保存 | 可。後から返った候補は捨てる |
| E38 | 読み取り 31 回目 | 429。失敗帯。手入力で保存できる |
| E39 | ボトル選択中に AI が種類を返す | 種類は変えない。品名・識別 4 項目は空欄、または直前の AI 値だけ |
| E40 | 確度 0.49 | 入れない |
...
| 入力検証 | Zod。評価 10〜50 の 1 刻み、品名 1〜100、識別 ≦100、4 欄 ≦2000、`photoIds` ≦6、`tastedOn` 暦日、`limit` ≦100 |
| XSS | 品名・識別・4 欄はテキスト。`dangerouslySetInnerHTML` 禁止。URL 化しない |
...
| 出典 | 項目 | 決定 | 根拠 |
|---|---|---|---|
| 5-01 | 評価の内部表現 | **`ratingX10` 整数 10〜50、1 刻み**。API に float を置かない | [data-model.md](../data-model.md) 5.5。float 比較を避ける |
| 5-01 | 1.0 未満（0 / 0.5） | **不可**。下限は 1.0（10） | data-model 5.5、画面バリデーション「10〜50」 |
| 5-01 | 評価 UI | **星 5 + スライダー + 左右 ± + 数値表示**（2026-09-14 改定。0.1 刻み。以前は 0.5 刻みで ± なし）。スライダーが主で星が連動。± で 0.1 増減。星タップは整数のショートカット | [05-notes.md](../screen-designs/05-notes.md) N5 |
| 5-01 | 評価の既定 | **未選択**（3.0 などを入れない）。保存必須 | 同上 |
| 5-01 | ボトル未選択時 | `drinkName`（1〜100）と `drinkType`（12 種）が **必須**。`bottleId` は null | data-model 6.4、N3 |
...
| 本仕様 | ヴィンテージ | ノートに列を持つ。常時入力。ボトル選択時は空欄ならクライアントがボトルの年を埋める。サーバーはボディがあれば採用、省略時はボトルからコピー | 2026-09-08 オーナー指示 |
| 本仕様 | ノート写真の AI | `POST /api/tasting-notes/recognize`。品名・種類・識別 4 項目。空欄と直前の AI 値は再読取で上書き。確度 0.5 未満は捨てる。上限はセラー・記録と共有 | 2026-09-08 オーナー指示。[register-identity.md](register-identity.md) |
