...
| L9 | 作成 | 右下 FAB `plus`（円 52px）→ `/notes/new`（`bottleId` があれば引き継ぐ）。撮影しない。[00-common.md](../screen-designs/00-common.md) 1.4 | — |
...
| N2 | セラーから選ぶ | 行「選ぶ ›」→ ダイアログ（検索 + 棚サムネ行、貯蔵庫含む） | `?bottleId=` があれば事前選択 | 種類の次。入力補助。未登録も下の欄で入力できる。空欄の品名・種類・識別だけ埋める（ユーザー修正は上書きしない）。行ラベルは棚なら「銘柄（セラー）」、貯蔵庫なら「銘柄（貯蔵庫）」。× で解除。既存のボトル関連付けは維持。`?fromLog=` は記録の識別・日付を先に入れる |
| N3 | 品名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。品名 1〜100。種類は 12 種。AI は空欄または直前の AI 値（種類は未操作）のとき入れる |
| N3b | 識別 | ヴィンテージ / 品種 / 生産者 / 生産国 | 空 | **常時表示**。任意。ヴィンテージは 1800〜2100 / 空 = NV。生産国は実在国の日本語名から選ぶ。他は ≦100。ボトル選択時は空欄ならボトルから。AI は空欄、または直前の AI 値を再読取で上書き。根拠不足は空欄 |
...
| N5 | 評価 | 星 5 + スライダー + 数値表示 | **未選択** | 1.0〜5.0、0.5 刻み。スライダー（`input type=range` step 0.5）が主で星が連動して埋まる。星タップは整数。同じ星の再タップで +0.5（5.0 は上限）。キーボード・読み上げはスライダー。0 として保存しない。数値入力欄・大きく離れた ± は置かない（2026-09-10） |
...
| N8 | ノートを保存 | Button 主（固定バー） | 無効 | 無効: 評価未選択 / 品名空（ボトル未選択時） / 種類未選択（ボトル未選択時） / 飲んだ日が不正・未来 / 写真アップロード中・失敗中 / 保存中。無効時「必須項目を入力してください」。未操作欄に初期エラーを並べない |
...
| `vintage` | 任意。整数 1800〜2100、または省略 / null（NV）。ボトルありでも **送った値を採用**（省略時は作成でボトルからコピー） | 「1800以上2100以下のヴィンテージを入力してください」 |
...
- スライダー: `input type=range` の 1.0〜5.0 / step 0.5 を `ratingX10FromSlider` で整数化（範囲外・0.5 未満刻みは捨てる）。塗りは `ratingSliderFillRatio`（可動域に対する 0〜1。未選択は 0）
- 星タップ: 整数 1〜5（`ratingX10 = 10 * n`）。すでにその整数のとき、タップし直しても 0 にはしない（未選択へ戻さない。クリアはしない）。同じ星の再タップで +0.5
- `stepRatingX10`（±0.5、未選択からは 1.0）は共有ヘルパとして残す（UI のステッパーは置かない）
...
| 入力検証 | Zod。評価 10〜50 の 5 刻み、品名 1〜100、識別 ≦100、4 欄 ≦2000、`photoIds` ≦6、`tastedOn` 暦日、`limit` ≦100 |
...
| 5-01 | 評価 UI | **星 5 + スライダー + 数値表示**（2026-09-10 改定。以前は星 + 数値 + ステッパー）。スライダーが主で星が連動。星タップは整数のショートカット | [05-notes.md](../screen-designs/05-notes.md) N5 |
