...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」+ サムネ 96×120 × 最大 6 | なし | **フォーム先頭**。「写真を撮る」→ `photo-edit`（4:5、`table`、**キャラ合成トグルあり**）。「写真を選ぶ」→ 保存済み写真。「使う」直後に未紐付け `POST /api/photos`。切り抜く前の 4:5 JPEG で `POST /api/tasting-notes/recognize`（銘柄・種類・ビンテージ。確度 0.5 未満は捨てる。触った欄は上書きしない。ボトル選択中は種類を変えない）。**自動起動しない**（`?camera=1` も無視）。7 枚目は撮る／選ぶを無効化し「写真は 6 枚までです」 |
...
| N2 | セラーから選ぶ | 行「選ぶ ›」→ ダイアログ（検索 + 棚サムネ行、貯蔵庫含む） | `?bottleId=` があれば事前選択 | 入力補助。未登録も下の欄で入力できる。空欄の銘柄名・種類・ビンテージだけ埋める（ユーザー修正は上書きしない）。行ラベルは棚なら「銘柄（セラー）」、貯蔵庫なら「銘柄（貯蔵庫）」。× で解除。既存のボトル関連付けは維持 |
| N3 | 銘柄名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。銘柄 1〜100。種類は 7 種。AI は空欄（種類は未操作）のときだけ入れる |
| N3b | ビンテージ | Input | 空（プレースホルダ NV） | **常時表示**。任意。1800〜2100 の整数、または空 = NV。ボトル選択時は空欄ならボトルの年を埋める。AI も空欄にだけ入れる |
| N4 | 飲んだ日 | ラベル上 + 行 → ネイティブ `date` | 今日（JST） | `YYYY-MM-DD`。今日は可。**未来不可**。端末 TZ に依存しない |
...
| N8 | ノートを保存 | Button 主（固定バー） | 無効 | 無効: 評価未選択 / 銘柄名空（ボトル未選択時） / 種類未選択（ボトル未選択時） / 飲んだ日が不正・未来 / 写真アップロード中・失敗中 / 保存中。無効時「必須項目を入力してください」。未操作欄に初期エラーを並べない |
...
| `vintage` | 任意。整数 1800〜2100、または省略 / null（NV）。ボトルありでも **送った値を採用**（サーバーはボトルから上書きしない） | 「1800以上2100以下のビンテージを入力してください」 |
...
| 出力 | `drinkName` ≦100、`drinkType` 7 種、`vintage` 1800〜2100。検証落ちは省く。`fields` 空でも 200 |
...
| E39 | ボトル選択中に AI が種類を返す | 種類は変えない。銘柄・ビンテージは空欄だけ |
| E40 | 確度 0.49 | 入れない |
...
| 本仕様 | ビンテージ | ノートに列を持つ。常時入力。ボトル選択時は空欄ならクライアントがボトルの年を埋める。サーバーはボトルから上書きしない | 2026-09-08 オーナー指示 |
| 本仕様 | ノート写真の AI | `POST /api/tasting-notes/recognize`。銘柄・種類・ビンテージ。空欄だけ。確度 0.5 未満は捨てる。上限はセラー・記録と共有 | 2026-09-08 オーナー指示 |
