...
| 写真付き | 一覧の右下「＋」→ フォーム → 「写真を撮る」（4:5 / `table` / キャラ合成トグル）→ 評価 → 保存。最大 6 枚。「写真を選ぶ」で保存済み写真も付けられる |
...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」+ サムネ 96×120 × 最大 6 | なし | **フォーム先頭**。「写真を撮る」→ `photo-edit`（4:5、`table`、**キャラ合成トグルあり**）。「写真を選ぶ」→ 保存済み写真。「使う」直後に未紐付け `POST /api/photos`。切り抜く前の 4:5 JPEG で `POST /api/tasting-notes/recognize`（品名・種類・識別 4 項目。確度 0.5 未満は捨てる。空欄と直前の AI 値は再読取で上書き。触った欄は上書きしない。ボトル選択中は種類を変えない。生産国・品種は根拠があるときだけ）。撮影日をテイスティング日の既定にする（`fromLog` と編集では維持）。**自動起動しない**（`?camera=1` も無視）。7 枚目は撮る／選ぶを無効化し「写真は 6 枚までです」 |
...
| N5 | 評価 | 星 5 + 数値入力 | **未選択** | 1.0〜5.0、0.5 刻み。星タップは整数。同じ星の再タップで +0.5（5.0 は上限）。数値欄 step 0.5 でキーボード操作。0 として保存しない。大きく離れた ± は置かない |
