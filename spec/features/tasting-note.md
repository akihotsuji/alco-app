...
- 状態: **承認済み**（5-01 #46）。5-02 CRUD / 5-03 複数写真 / 5-04 セラー連携 実装済み。5-05 テスト総仕上げ済み
...
| 写真付き | 一覧の「+」→ 撮影（4:5 / `table` / キャラ合成トグル）→ 評価 → 保存。最大 6 枚。フォームでは「選ぶ」で保存済み写真も付けられる |
...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真ストリップ | 「撮る」「選ぶ」タイル + サムネ 96×120 × 最大 6。横スクロール。先頭は撮る＋選ぶ | なし | 「撮る」→ 撮影で `photo-edit`（4:5、`table`、**キャラ合成トグルあり**）。「選ぶ」→ 保存済み写真。「使う」直後に未紐付け `POST /api/photos`。`?camera=1` ならマウント直後に撮影から開く（× でフォームへ）。7 枚目は撮る／選ぶを無効化し「写真は 6 枚までです」 |
...
| E17 | 写真 6 枚 | 撮る／選ぶタイル無効 +「写真は 6 枚までです」 |
