...
| # | 項目 | 入力 | 初期値 | 規則 |
|---|---|---|---|---|
| N1 | 写真（任意） | 「写真を撮る」「写真を選ぶ」+ サムネ 96×120 × 最大 6 | なし | 「写真を撮る」→ `photo-edit`（4:5、`table`、**キャラ合成トグルあり**）。「写真を選ぶ」→ 保存済み写真。「使う」直後に未紐付け `POST /api/photos`。**自動起動しない**（`?camera=1` も無視）。7 枚目は撮る／選ぶを無効化し「写真は 6 枚までです」 |
...
| N2 | セラーから選ぶ | 行「選ぶ ›」→ ダイアログ（検索 + 棚サムネ行、貯蔵庫含む） | `?bottleId=` があれば事前選択 | 入力補助。未登録も下の欄で入力できる。空欄の銘柄名・種類だけ埋める（ユーザー修正は上書きしない）。行ラベルは棚なら「銘柄（セラー）」、貯蔵庫なら「銘柄（貯蔵庫）」。× で解除。既存のボトル関連付けは維持 |
| N3 | 銘柄名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。銘柄 1〜100。種類は 7 種 |
