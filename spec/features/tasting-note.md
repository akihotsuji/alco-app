...
| 写真付き | 一覧の「＋ 作成」→ フォーム → 「写真を撮る」（4:5 / `table` / キャラ合成トグル）→ 評価 → 保存。最大 6 枚。「写真を選ぶ」で保存済み写真も付けられる |
...
| L8 | `bottleId` 指定時 | ヘッダーは ← + 銘柄名（ボトル詳細へ戻る）。一覧はそのボトルのみ。「＋ 作成」は `bottleId` を引き継ぐ（`/notes/new?bottleId=`） | `GET /api/tasting-notes?bottleId=` |
| L9 | 作成 | ヘッダー右「＋ 作成」→ `/notes/new`（`bottleId` があれば引き継ぐ）。撮影しない | — |
...
| N3 | 品名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。品名 1〜100。種類は 7 種。AI は空欄（種類は未操作）のときだけ入れる |
| N3b | 識別 | ヴィンテージ / 品種 / 生産者 / 生産国 | 空 | **常時表示**。任意。ヴィンテージは 1800〜2100 / 空 = NV。他は ≦100。ボトル選択時は空欄ならボトルから。AI も空欄にだけ入れる |
...
クライアント: 確度 0.5 未満は捨てる。空欄にだけ入れる。ボトル選択中は種類を変えない。新規・編集の両方で使う。
...
| E39 | ボトル選択中に AI が種類を返す | 種類は変えない。品名・識別 4 項目は空欄だけ |
| E40 | 確度 0.49 | 入れない |
...
| 本仕様 | ノート写真の AI | `POST /api/tasting-notes/recognize`。品名・種類・識別 4 項目。空欄だけ。確度 0.5 未満は捨てる。上限はセラー・記録と共有 | 2026-09-08 オーナー指示。[register-identity.md](register-identity.md) |
