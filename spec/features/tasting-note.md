...
| N3 | 品名 / 種類 | Input + セレクト | 空 / なし | **常に表示**。ボトル未選択時は必須。品名 1〜100。種類は 7 種。AI は空欄または直前の AI 値（種類は未操作）のとき入れる |
| N3b | 識別 | ヴィンテージ / 品種 / 生産者 / 生産国 | 空 | **常時表示**。任意。ヴィンテージは 1800〜2100 / 空 = NV。他は ≦100。ボトル選択時は空欄ならボトルから。AI は空欄、または直前の AI 値を再読取で上書き。生産国・品種はラベル情報から推測 |
...
| 出力 | `drinkName` ≦100、`drinkType` 7 種、`producer` / `origin` / `variety` ≦100、`vintage` 1800〜2100。検証落ちは省く。`fields` 空でも 200。`name` / `drinkName` はどちらも品名 |
...
| キャラ合成 | **あり**（トグル出す。既定 ON。`photo.mascot`）。右下。編集画面オープン時に 4 ポーズから 1 つ抽選。背後グローは敷かない |
| 切り抜き（背景除去） | **しない**（セラーのみ） |
