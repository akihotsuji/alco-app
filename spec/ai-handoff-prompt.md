...
- `PHOTO_UPLOAD_DAILY_LIMIT = 80`（ユーザー / JST 日。UI / PP に数値を出さない）。AI日次 30 回とは別枠
...
- Query `staleTime` は 30秒（`query-client.ts`）。写真は `Cache-Control: private, max-age=300`。未導入として足さない。
