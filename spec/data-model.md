...
| 評価 | 整数 `rating_x10`（10〜50、1 刻み）。1.0〜5.0 の 0.1 刻み | float 比較を避ける |
...
要件: 総合評価は 5 段階 × 0.1 刻み。

| UI | 保存値 |
|---|---|
| 1.0 … 5.0 | 10 … 50（1 刻み） |

0.5 や 0、5.5 は Zod で拒否。1.1（11）は可。表示は `rating_x10 / 10`。
...
| ratingX10 | rating_x10 | integer | NO | 10〜50、1 刻み | 総合評価 |
| createdAt | created_at | integer | NO | | |
| updatedAt | updated_at | integer | NO | | |
...
- 更新時の再同意ゲートは持たない（2026-09-22 決定。公開前で利用者はオーナーのみ）
- クライアントが送る時刻は使わない
