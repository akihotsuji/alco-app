...
| ラベル読み取り | セラーは切り抜く**前**の 2:3 JPEG を `POST /api/bottles/recognize` へ。記録はキャラ合成**前**の **全体 JPEG** を `POST /api/drink-logs/recognize` へ。ノート識別は記録側の読取に寄せ、`POST /api/tasting-notes/recognize` は置かない。画像は保存しない。撮影日は EXIF → `File.lastModified` で日付欄の既定（AI には送らない） | [04-cellar.md](04-cellar.md) B2 / [03-log.md](03-log.md) N1 / [ai-recognition.md](../features/ai-recognition.md) |
...
| P5c | 角度を調整 | Chip + スライダー | **セラーの切り抜き成功時だけ**。合計角度 -180〜180、1 度刻み。±1・90 度回転・0 度へ戻す。スライダーはラベル・現在値・キーボード操作あり。パネルを開くとプレビュー枠を縮め、90 度ボタンが保存バーに隠れないようにする。自動で直立にした場合は「自動で直立にしました」+「自動補正を取り消す」。角度変更は同じ RGB / マスクの再合成だけで、推論・アップロード・ラベル読み取りは発火しない。プレビューは rAF で集約し、角度のたびに WebP へはしない |
...
- 推論は **同一画像・同一 ROI・同一モデル／前処理で原則 1 回**。角度・棚配置の変更では再推論しない。プレビューで求めたマスクを「使う」で再利用する（Issue #48）
