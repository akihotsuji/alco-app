...
| 項目 | 決定 | 理由 |
|---|---|---|
| 取り込み | `<input type="file" accept="image/*">`（**`capture` は付けない**。2026-09-07 Issue #55）。OS の選択 UI で「カメラで撮る」と「保存済みの写真から選ぶ」のどちらも選べる。記録・セラー・ノートの全入口で同じ。`getUserMedia` は使わない | PWA スタンドアロン（iOS Safari / Android Chrome）で最も確実。権限 UI が OS 側で完結。端末に保存済みの写真も同じ導線で使える |
...
| P2 | 撮り直す | テキストボタン | `input[type=file]` を再度開く（OS の選択 UI。撮影でもライブラリでもよい） |
...
| 状態 | 表示 |
|---|---|
| ファイル選択待ち | OS の選択 UI（カメラ / 写真ライブラリ / ファイル）。キャンセルで `photo-edit` は開かない |
...
- [ ] `input[type=file]`（`capture` なし）で撮影または保存済み写真の選択 → `photo-edit` が開く
