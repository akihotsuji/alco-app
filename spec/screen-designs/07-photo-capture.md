...
| P5c | 角度を調整 | Chip + スライダー | **セラーの切り抜き成功時だけ**。合計角度 -180〜180、1 度刻み。±1・90 度回転・0 度へ戻す。スライダーはラベル・現在値・キーボード操作あり。自動で直立にした場合は「自動で直立にしました」+「自動補正を取り消す」。角度変更は同じ RGB / マスクの再合成だけで、推論・アップロード・ラベル読み取りは発火しない。プレビューは rAF で集約し、角度のたびに WebP へはしない |
