...
- 6-01: Service Worker（`/sw.js`）は静的アセット。`_headers` で `Cache-Control: no-cache`。`/api/*` は Workbox の NetworkOnly（セッション JSON と認可付き写真を SW キャッシュしない）。登録スクリプトはバンドル JS（`virtual:pwa-register`）で、`script-src` に `'unsafe-inline'` は足さない。詳細は [features/pwa.md](features/pwa.md)
