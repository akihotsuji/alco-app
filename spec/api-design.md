...
| タイムアウト | プロファイルに従う（Gemini 25 秒 / Llama 20 秒）。表 + 裏の 2 枚は別枠（3.7 Flash 40 秒）。超過は 502 `upstream_error` |
| ログ | 件数・所要時間・成否・画像枚数・失敗理由の要約・終了理由・トークン数のみ。画像・出力テキスト（読み取った値）をログに出さない |
