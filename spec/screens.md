...
| ID | 画面 | パス | 親タブ | 備考 |
|---|---|---|---|---|
| settings | 設定 | `/settings` | settings | ログアウト必須。表示名は任意（インライン編集）。メールは表示のみ。写真の既定 2 スイッチ、記録（現在地を記録する）、操作（触感・動きを減らす・使い方を見る→扇で記録/セラー/ノート）は端末保存。「このアプリ」から利用規約・PP。テーマ切替・招待コードは置かない |
