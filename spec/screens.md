...
| 下部タブ順 | **ホーム / セラー / 飲酒を記録（中央） / 友達 / 設定**。中央は円形 primary で浮かせる。旧ノートタブ位置を友達に置換（2026-09-20） | 2026-09-05 オーナー指示（1-07。2026-09-06 承認）。2026-09-14 に下ラベルを「飲酒を記録」へ |
| 中央タブの挙動 | **飲酒を記録（作成ボタン）**。円内は Plus。下ラベルは「飲酒を記録」。画面見出しは「お酒を記録」。タップで空の `log-new`（`/logs/new`）を開く。カメラ・写真選択は開かない。着地画面・現在地ハイライトなし | 2026-09-08 オーナー指示。下ラベルから「＋」を外す（2026-09-08）。2026-09-14 に文言を明確化 |
...
| セラーの見せ方 | 棚（陳列）。開栓で **味わい中**（セラー上部の立ちボトル列）、「飲み切った」で貯蔵庫（`/cellar/archive`）へ | 2026-09-05 オーナー指示（1-07）。2026-09-06 に開栓＝貯蔵庫へ。2026-09-24 に開栓＝味わい中へ見直し（[bottle-tasting.md](features/bottle-tasting.md)） |
...
| 項目 | 判断 | 理由 |
|---|---|---|
| タブ順 | ホーム → セラー → **飲酒を記録** → 友達 → 設定 | 記録が最頻（オーナー指示）。中央に置き、親指の届く位置で目立たせる。旧ノート位置を友達に置換（2026-09-20） |
| 中央タブの挙動 | **動作（着地なし）**。通常タブと区別した「飲酒を記録」作成ボタン。タップで `/logs/new` を写真なしで開く。カメラ・ファイル選択・権限要求は開始しない。現在地ハイライトなし。再タップもフォームへ（撮影は開始しない） | 2026-09-08。記録＝量を残す、写真＝フォーム内の任意操作。(c) 撮影開始は撤回 |
| 写真なし記録 | 中央タブ「飲酒を記録」→ `/logs/new`（写真はフォーム内で任意。保存 1 タップは維持）。ホームに記録ボタンは置かない | 記録の主導線は中央タブだけ。ホームはサマリー |
...
| 開栓 | 詳細の主ボタン。確認なし。成功後は詳細に留まり状態が味わい中になり、「飲んだ量を記録」だけ任意案内する。開栓だけでは記録もノートも作らない。味わい中の詳細に「飲んだ量を記録」「飲み切った」。飲み切りも確認なし（トーストで取り消し） | 2026-09-20。ノート専用ボタンは置かない。2026-09-24 に味わい中を追加 |
...
| 3（中央） | log | 飲酒を記録 | —（着地なし） | — | **着地しない**。`/logs/new`（写真なし）を開く。撮影は開始しない。再タップも同じ |
| 4 | friends | 友達 | `/friends` | `friends-feed` | 友達の近況 |
| 5 | settings | 設定 | `/settings` | `settings` | ログアウト・アカウント・写真の既定 |

中央タブは直径 52px の円形 primary ボタン（[screen-designs/00-common.md](screen-designs/00-common.md)）。中央タブは「動作」なので現在地ハイライトを持たない。他のタブの現在地ハイライトは「親タブ」列に従う（例: `/summary/week` と `/logs` 配下はホーム、`/cellar/archive` はセラー）。

ラベルは日本語のまま。中央「飲酒を記録」は 1 行（`nowrap`。「飲酒を記」「録」のようには切らない）。5 タブは iPhone SE（320px）で折り返すリスクがある。2-05 で 320px / 390px を確認し、切れ・折り返しがあれば design-system の字サイズで調整する。MVP ではアイコンのみタブにはしない。
...
| ID | 画面 | パス | 親タブ | 備考 |
|---|---|---|---|---|
| home | ホーム | `/` | home | 当日の杯数・純アルコール量、未記録状態、今週、味わい中（ボトル詳細へ）、直近マイドリンク。記録の入口は中央タブ |
...
| log-new | 記録入力 | `/logs/new` | home | 隠す | 中央タブ「飲酒を記録」は写真なしで開く。写真はフォーム内の任意操作。任意クエリ `?date=YYYY-MM-DD`（過去日）、`?camera=1`（無視。撮影しない）、`?bottleId=`（ボトル事前選択）、`?from=opened|detail`（詳細起点。戻る先はボトル詳細） |
...
| ID | 画面 | パス | 親タブ | タブバー | 備考 |
|---|---|---|---|---|---|
| bottle-list | セラー（棚） | `/cellar` | cellar | 表示 | 在庫（`sealed`）を棚 / 管理一覧で表示。上部に味わい中の立ちボトル列（補助表示）。種類・検索はクエリ。棚の見出しから `bottle-type-grid` |
| bottle-archive | 貯蔵庫 | `/cellar/archive` | cellar | 表示 | 飲み切ったボトル。飲み切り日の月ごと。`/cellar/:bottleId` より静的ルートを優先 |
...
| bottle-detail | ボトル詳細 | `/cellar/:bottleId` | cellar | 表示 | ヘッダーは「ボトル詳細」。品名は写真より上で全文。主「開栓する」はタブ直上。開栓後は任意の案内シート（「飲んだ量を記録」だけ）。詳細にも同じ 1 行。貯蔵庫は「開栓の記録を取り消す」。ノート節は一覧のみ（作成と混同しない）。中央タブは控えめ |
...
親タブは設定。ボトル詳細から `?bottleId=` のときはセラー。入口は設定「テイスティングノート」とボトル詳細。深いリンクは維持する。

| ID | 画面 | パス | 親タブ | タブバー | 備考 |
|---|---|---|---|---|---|
| note-list | ノート一覧 | `/notes` | settings | 表示 | 閲覧専用。FAB なし。`?bottleId=` ならセラー |
| note-detail | ノート詳細 | `/notes/:noteId` | settings | 表示 | 編集は親記録の `log-edit` |
...
| ID | 画面 | パス | 親タブ | 備考 |
|---|---|---|---|---|
| settings | 設定 | `/settings` | settings | ログアウト必須。表示名は任意（インライン編集）。メールは表示のみ。アカウント削除は `/settings/account/delete`。写真の既定 2 スイッチ、記録（現在地を記録する）、操作（触感・動きを減らす・使い方を見る→扇で記録/セラー/ノート）は端末保存。ノートの使い方は記録フォームのテイスティング節。「このアプリ」からご意見・ご要望・利用規約・PP。テーマ切替・招待コードは置かない |
...
| log-new | 中央タブ「飲酒を記録」（写真なし）。ボトル詳細・開栓後案内。写真はフォーム内の「写真を撮る / 選ぶ」。テイスティングは任意の折りたたみ | 履歴戻り（`from=opened|detail` ならボトル詳細。無ければその日の `log-day`） | 対象日の `log-day?highlight=`（保存後ダイアログは出さない） |
| log-edit | 日別の行タップ、ボトル詳細の記録行、ノート詳細の「編集」 | 対象日の `log-day` | 対象日の `log-day` |
| mydrink-list | ホームの「管理」 | ホーム | — |
| mydrink-new / mydrink-edit | 一覧の追加・行 | 一覧 | 一覧 |
| bottle-list | セラータブ、開栓後、復元後 | — | — |
| bottle-archive | 棚ヘッダー左の「貯蔵庫」 | 棚 | — |
| bottle-new | 棚の「＋ ボトル追加」（撮影から） | 棚 | 作成した最初の `bottle-detail` |
| bottle-batch | 棚の「まとめて追加」（撮影から） | 棚 | 棚（`/cellar`） |
| bottle-detail | 棚 / 貯蔵庫のボトル、ノート詳細のボトル行、記録の「セラーから」、開栓後の案内からの戻り | 棚（貯蔵庫の本は貯蔵庫） | 開栓成功後は詳細に留まる。記録・ノートは作らない。案内は「飲んだ量を記録」だけ |
| bottle-edit | 詳細の編集 | 詳細 | 詳細 |
| note-list | 設定「テイスティングノート」、詳細の「一覧」、ボトル詳細「すべて」 | — | — |
...
機能間連携のうち **開栓だけで記録やノートを自動作成しない**（2026-09-06 / 2026-09-08 / 2026-09-20）。開栓後の案内とボトル詳細から、任意で `log-new` をボトル付きで開ける（テイスティングも同じフォームの任意節）。保存後ダイアログ・`fromLog`・ノート単独作成は置かない。開栓 / ボトル詳細から記録へ進むときボトル写真を複製して入れる。
