...
| 中央タブの挙動 | **＋記録（作成ボタン）**。タップで空の `log-new`（`/logs/new`）を開く。カメラ・写真選択は開かない。着地画面・現在地ハイライトなし | 2026-09-08 オーナー指示。2026-09-06 の (c) 撮影開始は撤回 |
...
| log-new | 記録入力 | `/logs/new` | home | 隠す | 中央タブ「＋ 記録」とホームの「お酒を記録する」は写真なしで開く。ホームの「写真から記録」は撮影 → 「使う」の後に写真付き。任意クエリ `?date=YYYY-MM-DD`（過去日）、`?camera=1`（無視。撮影しない）、`?bottleId=`（ボトル事前選択） |
...
| note-new | ノート作成 | `/notes/new` | notes | 隠す | 任意クエリ `?bottleId=`。`?camera=1` は無視（自動起動しない）。写真 ≦6 |
...
| log-new | 中央タブ「＋ 記録」、ホームの「お酒を記録する」（いずれも写真なし）。ホームの「写真から記録」は撮影 → 「使う」の後に写真付き。ディープリンク | 履歴戻り（無ければその日の `log-day`） | 対象日の `log-day?highlight=`（今日は `/logs?highlight=<id>`、他は `/logs/:date?highlight=<id>`） |
