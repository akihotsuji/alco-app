...
| 下部タブ順 | **ホーム / セラー / 飲酒を記録（中央） / ノート / 設定**。中央は円形 primary で浮かせる | 2026-09-05 オーナー指示（1-07。2026-09-06 承認）。2026-09-14 に下ラベルを「飲酒を記録」へ |
...
| 項目 | 判断 | 理由 |
|---|---|---|
| タブ順 | ホーム → セラー → **飲酒を記録** → ノート → 設定 | 記録が最頻（オーナー指示）。中央に置き、親指の届く位置で目立たせる。2026-09-04 の「中央案不採用」を覆す |
...
| FAB | **セラー一覧・ノート一覧だけ**右下にピル「＋ ボトル追加」「＋ ノート作成」。記録の主導線は中央タブのまま。ホーム・設定・マイドリンク・詳細には置かない | 2026-09-07。2026-09-14 に文言付きピルへ。ヘッダー右の `plus` は廃止（マイドリンク一覧だけヘッダー右に残す） |
...
| ID | 画面 | パス | 親タブ | タブバー | 備考 |
|---|---|---|---|---|---|
| note-list | ノート一覧 | `/notes` | notes | 表示 | カード一覧。銘柄が主。検索・種類・評価の絞り込みはクエリ。`?bottleId=` でボトル起点。右下 FAB「＋ ノート作成」 |
...
| photo-edit | 写真を編集 | ルートなし（全画面オーバーレイ） | 記録・セラー・ノートの作成・編集から。`pushState` 1 段で戻るに反応。タブバー非表示。セラーの「切り抜きを修正」は同一オーバーレイのサブモード（別ルート・重ねモーダルなし）。戻る／Escape は先に修正モードを閉じる |
...
| log-new | 中央タブ「飲酒を記録」（写真なし）。ボトル詳細・開栓後案内。写真はフォーム内の「写真を撮る / 選ぶ」 | 履歴戻り（`from=opened|detail` ならボトル詳細。無ければその日の `log-day`） | 保存後ダイアログ。「つける」は `note-new?fromLog=`。「あとで」は対象日の `log-day?highlight=` |
...
| note-new | 一覧の右下「＋ ノート作成」、空状態の「ノートを作成」、ボトル詳細「書く」、`log-new` 保存後の「つける」（`?fromLog=`。いずれもフォームへ。撮影しない） | 履歴（無ければ一覧。`fromLog` は履歴戻り） | 作成した `note-detail` |
