...
| デモ用ボトル／グラス写真 | `public/fixtures/*.jpg` | 紹介素材用に生成した架空ラベルの静物写真。実在ブランドは使っていない | この紹介素材専用。再生成は `pnpm fixtures`（リポジトリルートで sharp を使う） |
| 実画面スクリーンショット | `public/shots/*.png` | ローカル検証環境に架空デモデータを投入し、Playwright で撮影 | 本番の個人情報・認証情報は使っていない |
