...
|---|---|
| 目安 | 2〜3日 |
| 状態 | 部分完了（GitHub と Cursor rules/skills のみ） |
...
- `wrangler dev` で Hello World がローカル起動する
...
| 03 | [Cloudflare D1/R2（dev）](03-cloudflare-dev-resources.md) | 未着手 |
| 04 | [空プロジェクト（Hello World）](04-hello-world-scaffold.md) | 未着手 |
...
推奨順: 01（済）→ 02 → 03 と 04 は並行しうるが、04 の `wrangler.jsonc` に 03 のリソース ID を書く → 05 → 06 → 07 → 08。09 は済。
...
- `package.json`・`src/`・`.gitignore`・`.github/`・`wrangler.jsonc` は存在しない
- GitHub リポジトリはプライベート
