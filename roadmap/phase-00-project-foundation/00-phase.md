...
|---|---|
| 目安 | 2〜3日 |
| 状態 | 部分完了（01〜06・09 済。残り 07 CI / 08 ブランチ保護） |
...
- ローカルツールチェーン（Node 22 / pnpm 10 / ローカル wrangler）は 0-02 で完了。`package.json`・`src/`・`wrangler.jsonc` は 0-04、TypeScript / Biome は 0-05、Vitest は 0-06。`.github/` は未着手。`.gitignore` は作成済み
