...
|---|---|
| フェーズ | Phase 8 一般公開準備 |
| ステータス | **完了**（2026-09-10。Google クライアント投入はオーナー） |
...
- ボタン「Googleアカウントでログインする」／「Googleアカウントで登録する」
- 失敗時は汎用エラー
...
- [x] Google でログインできる（モック callback。実クライアント投入はオーナー）
- [x] 自前 OAuth コードがない
- [x] secret が git に無い
- [x] 衝突方針が spec にある（暗黙リンクしない）
- [x] テストまたは手動手順が release-checklist 系にある
- [x] 監査
