...
|---|---|
| フェーズ | Phase 1 設計 |
| ステータス | **完了**（2026-09-06 オーナー承認。成果物: [spec/screen-designs/](../../spec/screen-designs/README.md)。中央タブの挙動は同日 (c) 撮影開始で確定） |
| 要件 | 全画面の機能・要素・状態・遷移をイメージ図付きで確定し、「設計どおりに実装する」決まりを作る |
| ソース | オーナー指示（2026-09-05 / 2026-09-06）: 画面設計とイメージ図を作り、実装はそのとおりに行う。記録タブを中央に。セラーは撮って陳列・追加と開栓・開栓で貯蔵庫へ。記録は記録画面で付ける。記録・ノートは写真を撮って付ける。キャラクターでポップに |
...
- data-model / api-design の改訂（開栓・貯蔵庫・記録写真）を同じ PR で行う
...
- 認証 / ホーム・サマリー / 記録（日別・入力・編集・マイドリンク）/ セラー（棚・貯蔵庫・追加・詳細・開栓・編集）/ ノート（グリッド・作成・詳細・編集）/ 設定 / 写真パイプライン
...
1. タブ順・中央タブの挙動を決める（当初は着地 `/logs` を既定にし代替案を要確認に残した。2026-09-06 に (c) 撮影開始で確定）
2. セラーの概念（追加 / 開栓 / 貯蔵庫 / 1 行 = 1 本）を決め、data-model / api に落とす
...
[spec/screen-designs/README.md](../../spec/screen-designs/README.md) の「1-07 で確定した設計判断」を正とする。要確認の結果（2026-09-06）:

| 項目 | 既定 | 結果 |
|---|---|---|
| 中央タブの挙動 | (a) 今日の日別 `/logs` に着地 | **確定: (c) タップで撮影開始**（`photo-edit` → 「使う」で写真付き `log-new`。着地・現在地ハイライトなし。日別はホーム配下）。(a) (b) は不採用。実装は 3-03 / 3-05 |
| 本数 N の上限 | 12 | **確定** |
| ノート写真枚数 | 6 | **確定** |

2026-09-05（2 回目）のオーナー決定で確定した点: 棚は **地色の上にガラス風の棚板 + 切り抜きボトル**（切り抜きを MVP へ、4-06）、**種類ごと / 1 本ずつ**の表示切替、ラベル読み取りは **Workers AI（Vision）** でセラーのみ（4-07。Gemini 等は将来）、デザイン崩れ（週マスの薄赤・チップの被り）は影トークンの 2 段階化で修正。

## 8. 受け入れ条件

- [x] 全画面（`screens.md` の MVP 画面 + `photo-edit`）に要素表・状態・遷移・受け入れチェックがある
...
- [x] オーナー承認（タブ順・棚・開栓・写真・キャラクター。2026-09-06。開栓＝貯蔵庫は同日追記）

## 9. セキュリティ観点

- ダミーデータのみ（実名・実店舗なし）
- 写真は端末内加工・サーバー magic bytes 検証・未紐付け GC を設計に含めた
- 開栓 / 復元 API は `id + user_id`、他人・不在・状態不一致は 404
