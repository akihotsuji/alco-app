...
- rule: `ui-design`（`src/client/**`）— 追加済み（1-03）。トークン使用、モバイルファースト、Win98 essences
...
- **トーン方針**（1-03 でトークン化）: Win98 寄りの essences を部分適用。使いやすさ最優先。ゲーミフィケーションはスコア表示と押下フィードバックに留める

## 要確認（フェーズ横断・残）

- なし（タブ数は上記で一旦確定。SE 幅の折り返しは 2-05 で検証）
