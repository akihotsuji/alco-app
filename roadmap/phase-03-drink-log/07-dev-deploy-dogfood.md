...
|---|---|
| フェーズ | Phase 3 飲酒記録 |
| ステータス | **完了**（2026-09-06。設定実装 + オーナーがリモート migrate / secret / deploy を実行。公開 URL はドキュメントに書かない） |
| 要件 | ドッグフーディング、無料枠 |
...
- [x] 実機で毎日の記録が運用できる（Phase 3 DoD）— 公開 HTTPS に載った。今夜からの利用はオーナー
- [x] CI グリーンのコードが載っている（設定 PR を載せるならマージ後に再デプロイ）
- [x] secret がリポジトリに無い
- [x] リモート migrate 済み
- [x] ロードマップ更新
