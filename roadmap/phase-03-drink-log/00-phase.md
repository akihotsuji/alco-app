...
|---|---|
| 目安 | 1〜1.5週間 |
| 状態 | 進行中（3-01 承認済み。3-04 / 3-02 完了。次は 3-03 → 3-05）。2026-09-06 に中央タブの挙動が **(c) 撮影開始** で確定し、日別の主ボタン群をホームへ集約（3-03 / 3-05 の要点を更新） |
...
| 03 | [マイドリンク](03-my-drinks.md) | プリセット登録と 1 タップ記録（**ホームのみ**）、ホームのキャラクター。1 タップの演出（M-07〜M-09 / M-17 / M-18）。**ホームの残りの導線**: 今日カード → 今日の日別、「今週 ›」→ 週、週マス → 日別（X6）。中央タブ / カメラ円ボタンの撮影開始は **完了**（2026-09-06。`feature/center-tab-camera`） |
