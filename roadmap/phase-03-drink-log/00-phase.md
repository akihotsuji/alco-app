...
|---|---|
| 目安 | 1〜1.5週間 |
| 状態 | 進行中（3-01 / 3-04 / 3-02 / 3-03 完了。次は 3-05）。2026-09-06 に中央タブの挙動が **(c) 撮影開始** で確定し、日別の主ボタン群をホームへ集約（3-03 / 3-05 の要点を更新） |
...
| 05 | [日別ビュー](05-daily-view.md) | **ホームから入る日別**（今日カード / 週マス / 保存後 / 週サマリーの行。中央タブの着地ではない）。当日一覧（写真サムネ）・合計・編集・削除・`?highlight=`（行の挿入 `useHighlightRow` M-14〜M-16、X1、X2、X7）。最上部の記録・カメラ・マイドリンクは置かない |
| 06 | [週/月サマリー](06-weekly-monthly-summary.md) | グラフ（初回 1 回だけ伸びる M-19）、休肝日。入口はホームの「今週 ›」→ 週 → 「今月 ›」→ 月、月の行 → 週、週の行 → 日別 |
