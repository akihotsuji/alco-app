...
- `ai_usage` の日次カウントと上限 10000 回（429。許容上限 MAX）。失敗時は加算しない。タイムアウト 20 秒（502）
...
- [x] 実機で撮影 → 数秒で候補が空欄に入り、AI 印が付く。編集で印が消える（クライアント: 空欄のみ・確度 0.5・AI 印。実機のモデル精度はデプロイ後）
- [x] 失敗・上限・設定 OFF のいずれでもフォームはそのまま使える
- [x] 他人 / 未認証は 401。日次 31 回目は 429
- [x] モデル出力を Zod で検証し、壊れた出力でも 200 で空 `fields`（テスト）
- [x] [04-cellar.md](../../spec/screen-designs/04-cellar.md) の該当受け入れチェック（B2 / S5）
- [x] DoD 5 項
...
- 無料枠（日次 Neurons）はモデルによって消費が違う。上限 10000 回は許容上限 MAX。使用量を見て env で下げられる
