...
ボトルの作成・詳細・編集・削除を、設計どおりの画面で出す。棚（陳列）の見た目は 4-04、開栓・貯蔵庫・復元は 4-03。本タスク時点の `/cellar` は仮の一覧（棚の骨格だけ）でよい。
...
- `bottle-detail`: 写真 2:3 + 棚板、状態ピル、主「開栓する」（4-03 で結線。本タスクでは disabled ではなく **出さない**）、プロパティ（銘柄名・種類・年・産地など）、ノート節（Phase 5 まで空）、記録節
- `bottle-edit`: 本数なし、写真付け替え、削除（確認。ノート・記録は残る）
- `GET /api/bottles?view=&q=&drinkType=`（一覧 API。棚 UI は 4-04）
...
- shared Zod（`count`。`status` は PATCH 不可）、routes、pages
- テスト: 作成（N 行、写真複製）、401、他人 404、検索が自分の行のみ、`log-new` のピッカーが自分のボトルのみ

## 5. 細分化タスク

1. Zod と enum（`sealed` / `consumed`）
...
- 詳細の「開栓する」を 4-03 前に出すと壊れた導線になる。出さない
