...
- [x] `/cellar` がガラス棚。1 本ずつ = 3 列、種類ごと = ゴースト見出し + 横スクロール 1 段。切替が URL と `localStorage` に残る
- [x] `cutout` は棚板に立ち、`photo` は角 8px の長方形、写真なしは種類別シルエット
- [x] 空状態・フィルタ 0 件・追加読み込み
- [x] ライト／ダークで棚板が見える
- [x] [04-cellar.md](../../spec/screen-designs/04-cellar.md) の受け入れチェックのうち `bottle-list` 項目
- [x] DoD 5 項
